...
.. warning::
   ...
   **Inner bounds (xhatters) are valid regardless of the above.** The
   xhat path uses the average scenario only as a *source of a
...
builds a single *average scenario* (via the module's
``average_scenario_creator``; see below), solves it with the spoke's
configured solver, and then:

* on the outer-bound path, sends the solver's best dual bound on the
  average scenario as its first outer bound;
* on the xhat path, takes the average scenario's first-stage solution,
  evaluates it across all scenarios, and reports the expected cost as
  its first inner bound if it is feasible for all scenarios.

.. admonition:: Solver options and what is extracted as the bound
   :class: note

   The average-scenario solve uses ``iterk_solver_options`` (the
   spoke's "production" tolerances), not ``iter0_solver_options``. It
   is a one-shot deterministic solve, not a first-iteration subproblem
   solve.

   On the outer-bound path the value sent to the hub is the **solver's
   best dual bound** on the average scenario (Pyomo's
   ``results.problem.lower_bound`` for minimize, ``upper_bound`` for
   maximize), not the incumbent objective value. With the dual bound,
   a non-zero MIP gap on the average-scenario solve does **not**
   invalidate Jensen's outer bound — the dual bound is a valid lower
   bound on the average-scenario optimum regardless of gap, and
   Jensen's inequality transitively makes it a valid lower bound on
   the expected recourse cost. (For an LP the dual bound and the
   incumbent coincide, so this distinction collapses.)
   ...
   the average-scenario solve there just means a possibly-worse
   candidate xhat, never an invalid bound.
...
**What the average is computed over.** The average-scenario data must
be the sample mean of the data for every scenario in the run (*not*
the expectation of any underlying continuous distribution). This
matches how ``mpi-sppy`` treats the outer problem: the sample-average
approximation (SAA) is what is actually being solved, and Jensen's
bound applies directly to that problem.

Authoring ``average_scenario_creator``
--------------------------------------
...
``average_scenario_creator`` functions. Please read this section before
writing your own.
...
With a local RNG, ``average_scenario_creator`` can safely be
...
sure ``average_scenario_creator`` accepts and forwards it too.
...
**Thread-safety.**
The ``average_scenario_creator`` you ship should be safe to call from
...
construction). ``average_scenario_creator`` averages the yield dicts
...
iteration 0, and the xhatshuffle spoke will evaluate the average
scenario's first-stage solution as its first xhat.
...
* **lagrangian**: its normal iteration 0 trivial bound (Lagrange
  ...
  that the average-scenario bound arrives *before* iteration 0
  finishes, which can matter a lot when there are many scenarios and
  iteration 0 is slow. The flag does not replace or disable iteration
  0. For some problems Jensen's bound is tighter than the trivial
  iteration 0 bound.
* **subgradient**: iteration 0 solves all scenarios. The average-
  scenario bound arrives earlier.
