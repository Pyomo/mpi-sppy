...
\begin{abstract}
Progressive hedging needs an outer bound on the optimal value.  The usual device
is to read the dual bound off the subproblem solver, which fails for Ipopt:
Ipopt is not a branch-and-bound solver and reports no dual bound.  This note
...
underestimator minimised in closed form over the variable box.  Its defining
...
\]
be the box of variable bounds.  With hub weights $\W_s$, the subproblem the
spoke solves is
