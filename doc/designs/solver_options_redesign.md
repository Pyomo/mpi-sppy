...
| `SPOpt.current_solver_options` (attribute reads / writes) | all internal readers migrated: fwph, aph, cross_scen_extension (PHBase-based) and xhat_eval, lshaped_bounder (SPOpt-based) now read via `_effective_solver_options(k)`; the attribute remains as the back-compat dynamic-overrides overlay path read by `_effective_solver_options` itself | reads can be deprecated next (only the back-compat overlay reads it now); writes (Gapper auto-mode, spoke iter0→iterk handoffs) can be migrated to the dynamic_gapper layer | future, TBD |
