...
sed -i 's/parser\.add_argument("--/config.add_to_config("/g' ${2}
# in case the first one did not hit; leave them with a bit of a mess...
sed -i 's/iparser\.add_argument/config.add_to_config/g' ${2}
...
sed -i "/config/s/(parser)/()/g" ${2}
sed -i "/config/s/(inparser)/()/g" ${2}
...
sed -i "s/args = parser\.parse_args()/parser = config\.create_parser(\'${1}\')\n    args = parser\.parse_args()\n    args = config\.global_config\.import_argparse(args)/g" ${2}
...
sed -i "s/return parser/parser = config\.create_parser('${1}')\n    args = parser\.parse_args()\n    args = config\.global_config\.import_argparse(args)/g" ${2}
...
echo
echo "And you probably need cfg = config.global_config in scopes where cfg is used"
