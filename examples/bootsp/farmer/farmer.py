...
###############################################################################
# A scalable "farmer" example for the bootstrap confidence-interval code. The
# crop yields fluctuate around a baseline according to a statdist univariate
# distribution (unif(0,1) by default, or a fitted distribution on the smoothed
# path), so importing this example needs the statdist library.
...
def xhat_generator(scenario_names, solver_name=None, solver_options=None, cfg=None):
    ...
    Args:
        ...
        cfg (Config): control parameters (crops_multiplier, yield_cv, ...)
    Returns:
