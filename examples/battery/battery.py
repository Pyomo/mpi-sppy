...
def scenario_creator(
    ...
    if (scenario_index < 0) or (scenario_index >= num_scenarios):
        raise RuntimeError(
            "Provided scenario index is invalid (must lie in "
    ...
    def flow_balance_constraint_rule(model, t):
        return (
            model.x[t + 1]
            == model.x[t] + data["eff"] * model.p[t] - (1 / data["eff"]) * model.q[t]
        )
    ...
    def big_M_constraint_rule(model, t):
        return (
            model.y[t] - model.q[t] + model.p[t]
            <= data["solar"][scenario_index, t]
            + data["M"][scenario_index, t] * model.z[0]
        )  # Why indexed??
    ...
    model.obj = pyo.Objective(
        expr=-pyo.dot_product(data["rev"], model.y)
