...
def abstract_model() -> pyo.AbstractModel:
    ...
    ab.times = pyo.Set(initialize=lambda mod: pyo.RangeSet(0, mod.time_horizon - 1))
    ...
    ab.lives_to_be_saved = pyo.Param(ab.times, ab.sites, within=pyo.NonNegativeIntegers)

    ab.rescue_times = pyo.Param(ab.times, ab.sites, within=pyo.NonNegativeIntegers)
    ...
    ab.depot_departures = pyo.Var(ab.times, ab.depots, ab.sites, domain=pyo.Binary)
    ab.stays_at_site = pyo.Var(ab.times, ab.sites, domain=pyo.Binary)
    ab.is_time_from_arrival = pyo.Var(ab.times, ab.times, ab.sites, domain=pyo.Binary)
    ...
    def depart_only_active_depots(mod, time, depot, site):
        return mod.depot_departures[time, depot, site] <= mod.is_active_depot[depot]
    ...
    def set_is_time_from_arrival(mod, time, time_from_arrival, site):
        ...
        if time > 0 and time_from_arrival + 1 < mod.time_horizon:
            num_teams_en_route += mod.is_time_from_arrival[
                time - 1, time_from_arrival + 1, site
            ]
        for d in mod.depots:
    ...
    ab.flow_conservation = pyo.Constraint(ab.times, ab.sites, rule=flow_conservation)
    ...
    def fully_service_site(mod, time, site):
        between_0_1_if_rescue_underway = (
            sum(
    ...
    ab.fully_service_site = pyo.Constraint(ab.times, ab.sites, rule=fully_service_site)
