...
def add_common_args(cnfg: ConfigDict) -> None:
    ...
    for name, domain, desc in (
        ...
        ("time_unit_minutes", float, "Number of minutes (float) per time step"),
        ("num_depots", int, "Number of depots (int) generated"),
        ("num_active_depots", int, "Number of depots (int) allowed to be active"),
        ("num_households", int, "Number of households (int) generated"),
        ("constant_rescue_time", int, "Flat time (int) for each household rescue"),
        (
            "travel_speed",
            float,
        ...
        cnfg.declare(name, val).declare_as_argument(group=group, required=True)
