...
def pysp2_callback(
    ...
    def lines_up_and_down(stage_md_dict, enode):
        ...
        for this_branch in stage_md_dict.elements("branch"):
            if this_branch[0] in enode.LinesUp:
                ...
                raise RuntimeError(
                    "Branch (line) {} neither up nor down in scenario {}".format(
                        this_branch[0], scenario_name
                    )
                )
    ...
    def _egret_model(md_dict):
        ...
        if not convex_relaxation:
            ...
            pyomod, mdict = eac_relax.create_soc_relaxation(
                md_dict, include_feasibility_slack=True, use_linear_relaxation=False
            )
        return pyomod, mdict
    ...
    full_scenario_model.stage_models[1], model_dict = _egret_model(first_stage_md_dict)
    ...
    for stage in range(2, numstages + 1):
        ...
        full_scenario_model.stage_models[stage], model_dict = _egret_model(
            stage_md_dict
        )
    ...
    def aggregate_ramping_rule(m):
        ...
        for stage in range(1, numstages):
            retval += sum(
                (
                    full_scenario_model.stage_models[stage + 1].pg[this_gen]
                    - full_scenario_model.stage_models[stage].pg[this_gen]
                )
                ** 2
    ...
    full_scenario_model.objective = pyo.Objective(
        expr=ramp_coeff * full_scenario_model.ramping
        + sum(
            full_scenario_model.stage_models[stage].obj.expr
...
def scenario_denouement(rank, scenario_name, scenario):
    ...
    for gen in gens:
        ...
        previous_val = None
        for stage in stages:
            current_val = pyo.value(
                getattr(scenario, "stage_models_" + str(stage)).pg[gen]
            )
        ...
        for stage in stages:
            current_val = pyo.value(
                getattr(scenario, "stage_models_" + str(stage)).qg[gen]
            )
            if previous_val is None:
                print("%6.2f -->> " % current_val, end=" ")
            else:
...
if __name__ == "__main__":
    ...
    if allbutlast % n_proc != 0 and n_proc % allbutlast != 0:
        raise RuntimeError(
            "allbutlast={} must divide n_proc={}, or vice-versa".format(
                allbutlast, n_proc
            )
        )
    ...
    if nscen % n_proc != 0:
        raise RuntimeError(
            "nscen={} must be a multiple of n_proc={}".format(nscen, n_proc)
        )
    ...
    scenario_creator_kwargs["etree"] = ET.ACTree(
        number_of_stages,
        branching_factors,
        seed,
        acstream,
    ...
    ef = sputils.create_EF(
        scenario_names, pysp2_callback, scenario_creator_kwargs=scenario_creator_kwargs
    )
    ###solver.options["BarHomogeneous"] = 1
    if "gurobi" in solver_name:
        solver.options["BarHomogeneous"] = 1
    ...
    print(
        "EF objective value for case {}={}".format(
            pyo.value(casename), pyo.value(ef.EF_Obj)
        )
    )
