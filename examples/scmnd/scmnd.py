...
def scenario_creator(scenario_name, data_dir=None):
    ...
    """
    if data_dir is None:
        raise ValueError("kwarg `data_dir` is required for SCMND scenario_creator")
    ...
    model._mpisppy_node_list = [
        scenario_tree.ScenarioNode(
            "ROOT", 1.0, 1, model.FirstStageCost, [model.DesignArcsVar], model
        )
    ]
...
def inparser_adder(cfg):
    ...
    cfg.add_to_config("instance_name",
                        description="SCMND instance name (e.g., scmnd_20_120_40_20)",
                        domain=str,
                        default=None)                
    cfg.add_to_config("scmnd_data_path",
                        description="path to SCMND data (e.g., ./data)",
                        domain=str,
                        default=None)                
    ...
    if hasattr(cfg, "num_scens"):
        if cfg.num_scens != ns:
            raise RuntimeError(f"Argument num-scens={cfg.num_scens} does not match the number "
                               f"implied by instance name={ns} "
                               "\n(--num-scens is not needed for scmnd)")
    else:
        cfg.add_and_assign("num_scens","number of scenarios", int, None, ns)
    data_dir = os.path.join(cfg.scmnd_data_path, inst, "scenariodata")
...
def sample_tree_scen_creator(sname, stage, sample_branching_factors, seed,
    ...
    Args:
        ...
        given_scenario (Pyomo concrete model): if not None, use this to get data for earlier stages
        scenario_creator_kwargs (dict): keyword args for the standard scenario creator function
    Returns:
...
def id_fix_list_fct(s):
    """ specify tuples used by the classic (non-RC-based) fixer.
        ...
        Returns:
             i0, ik (tuples): one for iter 0 and other for general iterations.
                 ...
                 The threshold is on the square root of the xbar squared difference
                 nb, lb an bu an "no bound", "upper" and "lower" and give the number
    ...
    # iter0tuples = [
    #     fixer.Fixer_tuple(s.DesignArcsVar[i], th=None, nb=None, lb=None, ub=None)
    #     for i in s.DesignArcsVar
    # ]
    iterktuples = [
        fixer.Fixer_tuple(s.DesignArcsVar[i], th=0, nb=None, lb=None, ub=None)
        for i in s.DesignArcsVar
    ]
...
if __name__ == "__main__":
    msg = (
        ...
        + "then rho (e.g., scmnd_5_20_5_10 0 6 1)"
    )
