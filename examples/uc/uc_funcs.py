...
def scenario_names_creator(scnt,start=0):
    # (only for Amalgaator): return the full list of names
    return [F"Scenario{i+1}" for i in range(start,scnt+start)]

#=========
def inparser_adder(inparser):
    # (only for Amalgaator): add command options unique to uc
...
def kw_creator(options):
    # (only for Amalgaator and MMW_conf): linked to the scenario_creator and inparser_adder
    if "path" in options:
        path = options["path"]
