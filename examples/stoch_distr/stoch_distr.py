...
def min_cost_distr_problem(
    local_dict, cfg, stoch_scenario_name, sense=pyo.minimize, max_revenue=None
):
    ...
    if cfg.ensure_xhat_feas:
        ...
        model.SecondStageCost = pyo.Expression(
            ...
            - sum(
                local_dict["production costs"][n] / 2 * model.inventory[n]
                for n in local_dict["factory nodes"]
            )
        )
    else:
    ...
    model.MinCost = pyo.Objective(
        expr=model.FirstStageCost + model.SecondStageCost, sense=sense
    )
    ...
    def FlowBalance_rule(m, n):
        if n in local_dict["factory nodes"]:
            ...
            np.random.seed(
                node_num + cfg.initial_seed + (scennum + 1) * 2**20
            )  # 2**20 avoids the correlation with the scalable example data
            return sum(m.flow[a] for a in arcsout[n]) - sum(
                m.flow[a] for a in arcsin[n]
            ) + m.inventory[n] == (local_dict["supply"][n] - m.y[n]) * min(
...
def scenario_creator(
    ...
    model = min_cost_distr_problem(
        local_dict, cfg, stoch_scenario_name, max_revenue=data_params["max revenue"]
    )

    sputils.attach_root_node(
        model, model.FirstStageCost, [model.y[n] for n in local_dict["factory nodes"]]
    )
...
def scenario_denouement(
    rank, admm_stoch_subproblem_scenario_name, scenario, eps=10 ** (-6)
):
    ...
    for var in scenario.y:
        ...
        if "F" in var:
            if scenario.inventory[var].value > 10 * eps:
                print(
                    f"In {rank=} for {admm_stoch_subproblem_scenario_name}, the inventory {scenario.inventory[var].name} is big, its value is {scenario.inventory[var].value}. Although it is rare after convergence, it makes sense if in a scenario it is really interesting to produce."
                )
...
def split_admm_stoch_subproblem_scenario_name(admm_stoch_subproblem_scenario_name):
    ...
    assert (
        len(splitted) == 4
    ), "no underscore should be attached to admm_subproblem_name nor stoch_scenario_name"
