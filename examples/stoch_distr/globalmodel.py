...
def global_dict_creator(num_admm_subproblems, start=0):
    ...
    for a in inter_region_dict["capacities"]:
        global_dict["flow capacities"][_extract_arc(a)] = inter_region_dict[
            "capacities"
        ][a]
    return global_dict
...
def main():
    ...
    cfg = _parse_args()
    print(
        "won't give the good results because there is only one scenario, and the scennum on which are based the seeds are different"
    )
    model = stoch_distr.min_cost_distr_problem(
        global_dict_creator(cfg.num_admm_subproblems), cfg
    )
