...
def _count_cylinders(cfg):
    ...
    cfglist = [
        ...
        "lagrangian",
        "ph_ob",
    ]  # All the cfg arguments that create a new cylinders
...
def _make_admm(
    cfg, n_cylinders, all_nodes_dict, inter_region_dict, data_params, verbose=None
):
    options = {}

    admm_subproblem_names = stoch_distr.admm_subproblem_names_creator(
        cfg.num_admm_subproblems
    )
    ...
    admm = stoch_admmWrapper.Stoch_AdmmWrapper(
        options,
        all_admm_stoch_subproblem_scenario_names,
        split_admm_stoch_subproblem_scenario_name,
...
def _wheel_creator(
...
):  # the wrapper doesn't need any kwarg
    ph_converger = None
    ...
    if cfg.run_async:
        ...
        )  # needs to be modified, all_nodenames is None only for 2 stage problems
    ...
    assert n_cylinders == 1 + len(
        list_of_spoke_dict
    ), f"{n_cylinders=},{len(list_of_spoke_dict)=}"
...
def main(cfg):
    ...
    if cfg.scalable:
        ...
        with open(json_file_path, "r") as file:
            ...
            all_DC_nodes = [
                DC_node
                for region in all_nodes_dict
                for DC_node in all_nodes_dict[region]["distribution center nodes"]
            ]
            inter_region_dict = distr_data.scalable_inter_region_dict_creator(
                all_DC_nodes, cfg, data_params
            )

    else:
        inter_region_dict = distr_data.inter_region_dict_creator(
            num_scens=cfg.num_admm_subproblems
        )
        all_nodes_dict = None
        data_params = {
            "max revenue": 1200
        }  # hard-coded because the model is hard-coded

    n_cylinders = _count_cylinders(cfg)
    admm, all_admm_stoch_subproblem_scenario_names = _make_admm(
        cfg, n_cylinders, all_nodes_dict, inter_region_dict, data_params
    )

    scenario_creator = (
        admm.admmWrapper_scenario_creator
    )  # scenario_creator on a local scale
    ...
    if write_solution:
        ...
        wheel.write_first_stage_solution(
            "stoch_distr_cyl_nonants.npy",
            first_stage_solution_writer=sputils.first_stage_nonant_npy_serializer,
        )
        wheel.write_tree_solution("stoch_distr_full_solution")
    if global_rank == 0:
        best_objective = (
            wheel.spcomm.BestInnerBound
        )  # * len(all_admm_stoch_subproblem_scenario_names)
