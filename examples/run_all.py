...
if len(sys.argv) > 3:
    ...
    if sys.argv[3] != "nouc":
        raise RuntimeError(
            "Third arg can only be nouc (you have {})".format(sys.argv[3])
        )
...
def time_one(ID, dirname, progname, np, argstring):
    ...
    if os.path.isfile(listfname):
        ...
        timelistdf = pd.DataFrame(
            [[finish, refsecs, runsecs]], columns=["datetime", "reftime", "time"]
        )
    ...
    if len(timelistdf) > 0:
        ...
        if deltafrac > 0.1:
            print(
                f"**** WARNING: {100*deltafrac}% time increase for {ID}, see {listfname}"
            )
...
def do_one_mmw(dirname, runefstring, npyfile, mmwargstring):
    ...
    # run mmw, remove .npy file
        ...
        runstring = "python -m mpisppy.confidence_intervals.mmw_conf {} --xhatpath {} {}".format(
            dirname, npyfile, mmwargstring
        )
        code = os.system("echo {} && {}".format(runstring, runstring))
        if code != 0:
            if dirname not in badguys:
...
do_one("farmer", "farmer_ef.py", 1, "1 3 {}".format(solver_name))
...
do_one(
    ...
    "--init-W-fname=out_ws.txt --init-Xbar-fname=out_xbars.txt --ph-track-progress --track-convergence=4 "
    "--track-xbar=4 --track-nonants=4 --track-duals=4 ".format(solver_name),
)
do_one(
    "farmer",
...
do_one(
    ...
    "--default-rho=1 --solver-name={} --lagrangian --xhatshuffle --fwph".format(
        solver_name
    ),
)
do_one(
...
do_one(
    ...
    "--slammax --lagrangian --xhatshuffle --cross-scenario-cuts --max-solver-threads=2".format(
        solver_name
    ),
)
...
do_one_mmw(
    "farmer",
    f"python farmer_ef.py 3 3 {solver_name}",
    "farmer_cyl_nonants.npy",
    f"--MMW-num-batches=5 --confidence-level 0.95 --MMW-batch-size=10 --start-scen 4 --EF-solver-name={solver_name}",
)
