# general example driver for distr_scalable with cylinders
import mpisppy.utils.admmWrapper as admmWrapper
import distr_data
import distr_scalable
...
def _parse_args():
    ...
    cfg = config.Config()
    distr_scalable.inparser_adder(cfg)
    cfg.popular_args()
    cfg.two_sided_args()
    cfg.ph_args()
    cfg.aph_args()
...
def main():
    ...
    options = {}
    all_scenario_names = distr_scalable.scenario_names_creator(num_scens=cfg.num_scens)
    scenario_creator = distr_scalable.scenario_creator
    scenario_creator_kwargs = distr_scalable.kw_creator(all_nodes_dict, cfg, inter_region_dict, data_params)  
    consensus_vars = distr_scalable.consensus_vars_creator(cfg.num_scens, inter_region_dict, all_scenario_names)
    ...
    scenario_denouement = distr_scalable.scenario_denouement
