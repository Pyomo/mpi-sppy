...
def main():
    ...
    scenario_creator_kwargs = distr.kw_creator(
        all_nodes_dict, cfg, inter_region_dict, data_params
    )
    consensus_vars = distr.consensus_vars_creator(
        cfg.num_scens, inter_region_dict, all_scenario_names
    )
    n_cylinders = _count_cylinders(cfg)
    admm = admmWrapper.AdmmWrapper(
        options,
        all_scenario_names,
        scenario_creator,
        consensus_vars,
    ...
    )  ##change needed because of the wrapper
    ...
    assert (
        n_cylinders == 1 + len(list_of_spoke_dict)
    ), f"n_cylinders = {n_cylinders}, len(list_of_spoke_dict) = {len(list_of_spoke_dict)}"
