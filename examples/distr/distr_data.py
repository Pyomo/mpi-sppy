...
def _node_num(max_node_per_region, node_type, region_num, count):
    ...
    node_types = [
        ...
        "F",
        "B",
    ]  # no need to include the dummy nodes as they are added automatically
    return (max_node_per_region * region_num + count) * len(
        node_types
...
def _pseudo_random_arc(node_1, node_2, prob, cfg, intra=True):  # in a Region
    ...
    if intra:
        assert (
            region_num1 == region_num2
        ), f"supposed to happen in a region ({intra=}), but {region_num1, region_num2=}"
    else:
        if (
            region_num1 == region_num2
        ):  # inside a region, however intra=False so no connexion
            return False
    max_node_num = _node_num(
        max_node_per_region, "B", cfg.num_scens + 1, max_node_per_region + 1
    )  # maximum number possible
    np.random.seed(
        min(node_num1, node_num2) * max_node_num + max(node_num1, node_num2)
    )  # it is symmetrical
...
def _intra_arc_creator(
    my_dict, node_1, node_2, cfg, arc, arc_params, my_seed, intra=True
):
    ...
    if _pseudo_random_arc(node_1, node_2, prob, cfg, intra=intra):
        ...
        my_dict[capacity_name][arc] = max(
            np.random.normal(mean_capacity, cv_capacity), 0
        )


def scalable_inter_region_dict_creator(
    all_DC_nodes, cfg, data_params
):  # same as inter_region_dict_creator but the scalable version
...
def all_nodes_dict_creator(cfg, data_params):
    ...
    for i in range(1, num_scens + 1):
        ...
        for node_type in node_types:
            node_base_num = _node_num(
                max_node_per_region, node_type, i, 1
            )  # the first node that will be created will have this number
            ...
            all_nodes_dict[region_name][association_types[node_type]] = [
                node_type + str(i) + "_" + str(j) for j in range(1, m + 1)
            ]
            all_nodes_dict[region_name]["nodes"] += all_nodes_dict[region_name][
                association_types[node_type]
            ]
            if node_type == "F":
                count = 1
                for node_name in all_nodes_dict[region_name][
                    association_types[node_type]
                ]:
                    np.random.seed(
                        _node_num(max_node_per_region, node_type, i, count) + 2**28
                    )
                    all_nodes_dict[region_name]["production costs"][node_name] = max(
                        0, np.random.normal(production_costs_mean, production_costs_cv)
                    )
                    np.random.seed(
                        _node_num(max_node_per_region, node_type, i, count) + 2 * 2**28
                    )
                    all_nodes_dict[region_name]["supply"][node_name] = max(
                        0, np.random.normal(supply_factory_mean, supply_factory_cv)
                    )  # positive
                    count += 1
            if node_type == "B":
                count = 1
                for node_name in all_nodes_dict[region_name][
                    ...
                    all_nodes_dict[region_name]["revenues"][node_name] = min(
                        max(0, np.random.normal(revenues_mean, revenues_cv)),
                        data_params["max revenue"],
                    )
                    np.random.seed(
                        _node_num(max_node_per_region, node_type, i, count) + 2 * 2**28
                    )
                    all_nodes_dict[region_name]["supply"][node_name] = -max(
                        0, np.random.normal(supply_buyer_mean, supply_buyer_cv)
                    )  # negative
                    count += 1
            if node_type == "DC":
                for node_name in all_nodes_dict[region_name][
...
def scalable_region_dict_creator(
    scenario_name, all_nodes_dict=None, cfg=None, data_params=None
):  # same as region_dict_creator but the scalable version
    ...
    for node_1 in local_nodes_dict[
        "nodes"
    ]:  # although inter_region_dict["costs"] and ["capacities"] could be done with comprehension, "arcs" can't
        for node_2 in local_nodes_dict["nodes"]:
            if node_1 != node_2:
                node_type1, _, _ = parse_node_name(node_1)
                node_type2, _, _ = parse_node_name(node_2)
                arcs_association = {
                    ("F", "DC"): data_params["arc_F_DC"],
                    ("DC", "B"): data_params["arc_DC_B"],
                    ("F", "B"): data_params["arc_F_B"],
                    ("DC", "DC"): data_params["arc_DC_DC"],
                }
