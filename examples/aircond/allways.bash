...
mpiexec --oversubscribe -np 3 python -m mpi4py aircond_cylinders.py --max-iterations=10 --default-rho=1 --solver-name=${SOLVERNAME}  --rel-gap 0.01  --max-solver-threads 2 --start-seed $SS --no-fwph --no-lagranger --bundles-per-rank=0  --write-solution --intra-hub-conv-thresh 0 --branching-factors $BF1 $BF2 $EC

echo "======== Cylinders with ${BF1} bundles per rank (not proper bundles)"
mpiexec --oversubscribe -np 3 python -m mpi4py aircond_cylinders.py --max-iterations=10 --default-rho=1 --solver-name=${SOLVERNAME}  --rel-gap 0.01  --max-solver-threads 2 --start-seed $SS --no-fwph --no-lagranger --bundles-per-rank=$BF1  --write-solution --intra-hub-conv-thresh 0 --branching-factors $BF1 $BF2 $EC
...
python bundle_pickler.py --branching-factors $BF1 $BF2 --pickle-bundles-dir="." --scenarios-per-bundle=$SPB $EC --start-seed=$SS
