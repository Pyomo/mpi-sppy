...
def main():
    solution_files = {
        "first_stage_solution": "aircond_first_stage.csv",
        "tree_solution": "aircond_full_solution",
    }
    cfg = config.Config()
    cfg.quick_assign("mstage", bool, True)
    cfg.quick_assign(
        "cylinders", pyofig.ListOf(str), ["ph", "xhatshuffle", "lagrangian"]
    )
    ...
    if ama_options.get("num_scens", None) is not None:
        raise RuntimeError(
            "Do not use num_scens here, we want to solve the problem for the whole sample scenario tree"
        )
    ...
    ama = amalgamator.Amalgamator(
        cfg, scenario_names, aircond.scenario_creator, aircond.kw_creator
    )
    ama.run()
    if ama.on_hub:
