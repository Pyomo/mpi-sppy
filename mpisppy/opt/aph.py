...
logging.basicConfig(
    level=logging.CRITICAL,  # level=logging.CRITICAL, DEBUG
...
class APH(ph_base.PHBase):
    ...
    def setup_Lens(self):
        ...
        self.Lens = collections.OrderedDict({"FirstReduce": {}, "SecondReduce": {}})

        for sname, scenario in self.local_scenarios.items():
            for node in scenario._mpisppy_node_list:
                self.Lens["FirstReduce"][node.name] = 3 * len(node.nonant_vardata_list)
    ...
    def __init__(
        ...
        self.plot_trace_prefix = (
            options.get("APHplot_trace_prefix") if self.cylinder_rank == 0 else None
        )
        self.conv_trace_filename = (
            None
            if self.plot_trace_prefix is None
            else f"{self.plot_trace_prefix}_dyngam_{self.use_dynamic_gamma}"
    ...
    def Update_y(self, dlist, verbose):
        ...
        if self._PHIter != 1:
            for k, s in self.local_scenarios.items():
                if (not self.bundling and k in slist) or (
                    self.bundling and s._mpisppy_data.bundlename in slist
                ):
                    for (ndn, i), xvar in s._mpisppy_data.nonant_indices.items():
                        if not self.use_lag:
                        ...
                        s._mpisppy_model.y[(ndn, i)]._value = W_touse + pyo.value(
                            s._mpisppy_model.rho[(ndn, i)]
                        ...
                        if s._mpisppy_data.has_variable_probability:
                            s._mpisppy_model.y[
                                (ndn, i)
                            ]._value *= s._mpisppy_data.prob0_mask[ndn][i]
        else:
            for k, s in self.local_scenarios.items():
                for (ndn, i), xvar in s._mpisppy_data.nonant_indices.items():
    ...
    def compute_phis_summand(self):
        ...
        for k, s in self.local_scenarios.items():
            ...
            for (ndn, i), xvar in s._mpisppy_data.nonant_indices.items():
                # Step 16, phi
                self.phis[k] += (
                    pyo.value(s._mpisppy_model.z[(ndn, i)]) - xvar._value
                ) * (
                    pyo.value(s._mpisppy_model.W[(ndn, i)])
                    - pyo.value(s._mpisppy_model.y[(ndn, i)])
                )
    ...
    def listener_side_gig(self, synchro):
        ...
        self.synchronizer._unsafe_get_global_data("FirstReduce", self.node_concats)
        self.synchronizer._unsafe_get_global_data("SecondReduce", self.node_concats)
        ...
        logging.debug(
            "   +++ debug enter listener_side_gig on cylinder_rank {} last phi update {}".format(
                self.cylinder_rank, lptut
            )
        )
        ...
        for cr in range(self.n_proc):
            ...
            if self.node_concats["FirstReduce"]["ROOT"][-backdist] > lptut:
                xbarin += 1
        ...
        if fracin < self.options["async_frac_needed"]:
            ...
            logging.debug(
                "  ^ debug not good to go listener_side_gig on cylinder_rank {}; xbarin={}; fracin={}".format(
                    self.cylinder_rank, xbarin, fracin
                )
            )
            return
        ...
        logging.debug(
            "^^^ debug good to go  listener_side_gig on cylinder_rank {}; xbarin={}".format(
                self.cylinder_rank, xbarin
            )
        )
        if verbose and self.cylinder_rank == 0:
            print("(%d)" % xbarin)
        ...
        for k, s in self.local_scenarios.items():
            ...
            for ndn, i in s._mpisppy_data.nonant_indices:
                s._mpisppy_model.xbars[(ndn, i)]._value = self.node_concats[
                    "FirstReduce"
                ][ndn][i]
                s._mpisppy_model.xsqbars[(ndn, i)]._value = self.node_concats[
                    "FirstReduce"
                ][ndn][nlens[ndn] + i]
                s._mpisppy_model.ybars[(ndn, i)]._value = self.node_concats[
                    "FirstReduce"
                ][ndn][2 * nlens[ndn] + i]

                if verbose and self.cylinder_rank == 0:
                    print(
                        "rank, scen, node, var, xbar:",
                        self.cylinder_rank,
                        k,
                        ndn,
                        s._mpisppy_data.nonant_indices[ndn, i].name,
        ...
        for sname, s in self.local_scenarios.items():
            ...
            if not s._mpisppy_data.has_variable_probability:
                for (ndn, i), xvar in s._mpisppy_data.nonant_indices.items():
                    self.uk[sname][(ndn, i)] = xvar._value - pyo.value(
                        s._mpisppy_model.xbars[(ndn, i)]
                    )  # Eq.27
                    # compute the usqnorm and vsqnorm (squared L2 norms)
                    scen_usqnorm += self.uk[sname][(ndn, i)] * self.uk[sname][(ndn, i)]
                    scen_vsqnorm += pyo.value(
                        s._mpisppy_model.ybars[(ndn, i)]
            ...
            if not s._mpisppy_data.has_variable_probability:
                ...
                self.local_pusqnorm += (
                    pyo.value(s._mpisppy_probability) * scen_usqnorm
                )  # prob first done
                self.local_pvsqnorm += (
                    pyo.value(s._mpisppy_probability) * scen_vsqnorm
                )  # prob first done
        ...
        if self.global_tau <= 0:
            logging.debug(
                "  *** Negative tau={} on rank {}".format(
                    self.global_tau, self.cylinder_rank
                )
            )
    ...
    def Compute_Averages(self, verbose=False):
        ...
        if not hasattr(self, "local_concats"):
            ...
            self.local_concats = {
                "FirstReduce": {},  # keys are tree node names
                "SecondReduce": {},
            }
            self.node_concats = {
                "FirstReduce": {},  # concat of xbar and xsqbar
                "SecondReduce": {},
            }
            ...
            for k, s in self.local_scenarios.items():
                ...
                for node in s._mpisppy_node_list:
                    if node.name not in nodenames:
                        ...
                        self.local_concats["FirstReduce"][ndn] = np.zeros(
                            mylen, dtype="d"
                        )
                        self.node_concats["FirstReduce"][ndn] = np.zeros(
                            mylen, dtype="d"
                        )
            # second reduce is tau and phi
            mylen = self.Lens["SecondReduce"]["ROOT"]
            self.local_concats["SecondReduce"]["ROOT"] = np.zeros(mylen, dtype="d")
            self.node_concats["SecondReduce"]["ROOT"] = np.zeros(mylen, dtype="d")
        ...
        for k, s in self.local_scenarios.items():
            ...
            for node in s._mpisppy_node_list:
                ...
                for i in range(nlens[node.name]):
                    ...
                    logging.debug(
                        "  rank= {} scen={}, i={}, v_value={}".format(
                            global_rank, k, i, v_value
                        )
                    )
                    self.local_concats["FirstReduce"][node.name][nlens[ndn] + i] += (
                        (s._mpisppy_probability / node.uncond_prob) * v_value * v_value
                    )
                    self.local_concats["FirstReduce"][node.name][
                        2 * nlens[ndn] + i
                    ] += (s._mpisppy_probability / node.uncond_prob) * pyo.value(
                        s._mpisppy_model.y[(node.name, i)]
                    )
        ...
        self.local_concats["FirstReduce"]["ROOT"][
            3 * nlens["ROOT"] + self.cylinder_rank
        ] = secs_sofar
        logging.debug(
            "Compute_Averages at secs_sofar {} on rank {}".format(
                secs_sofar, self.cylinder_rank
            )
        )
        ...
        while (
            self.synchronizer.global_quitting == 0 and self.synchronizer.enable_side_gig
        ):
            # Other ranks could be reporting, so keep looking for them.
            self.synchronizer.compute_global_data(self.local_concats, self.node_concats)
            if not self.synchronizer.enable_side_gig:
                logging.debug(
                    " did side gig break on rank {}".format(self.cylinder_rank)
                )
                break
            else:
        ...
        logging.debug(
            "Assigned global tau {} and phi {} on rank {}".format(
                self.global_tau, self.global_phi, self.cylinder_rank
            )
        )
    ...
    def Update_theta_zw(self, verbose):
        ...
        if self.global_tau <= 0:
            logging.debug(
                "|tau {}, rank {}".format(self.global_tau, self.cylinder_rank)
            )
            self.theta = 0  # Step 17
        elif self.global_phi <= 0:
            logging.debug(
                "|phi {}, rank {}".format(self.global_phi, self.cylinder_rank)
            )
            self.theta = 0
        else:
            ...
            for k, s in self.local_scenarios.items():
                for (ndn, i), xvar in s._mpisppy_data.nonant_indices.items():
                    if punorm <= pvnorm:
                        ...
                        factor = 1 + rho_val
                    s._mpisppy_model.rho[(ndn, i)] = (
                        pyo.value(s._mpisppy_model.rho[(ndn, i)]) * factor
                    )
                    # print(f'rho={pyo.value(s._mpisppy_model.rho[(ndn,i)])}')

        logging.debug(
            "Iter {} assigned theta {} on rank {}".format(
                self._PHIter, self.theta, self.cylinder_rank
            )
        )
        ...
        for k, s in self.local_scenarios.items():
            ...
            for ndn, i in s._mpisppy_data.nonant_indices:
                ...
                Ws = (
                    pyo.value(s._mpisppy_model.W[(ndn, i)]) + Wupdate
                )  # Step 19, Algorithm 2
                ...
                if self._PHIter != 1:  # Step 18, Algorithm 2
                    ...
                    zs = (
                        pyo.value(s._mpisppy_model.z[(ndn, i)])
                        + self.theta
                        * pyo.value(s._mpisppy_model.ybars[(ndn, i)])
                        / self.APHgamma
                    )
                ...
                logging.debug(
                    "rank={}, scen={}, i={}, Ws={}, zs={}".format(
                        global_rank, k, i, Ws, zs
                    )
                )
    ...
    def Compute_Convergence(self, verbose=False):
        ...
        logging.debug(
            "self.conv={} self.global_pusqnorm={} self.global_pwsqnorm={} self.global_pvsqnorm={} self.global_pzsqnorm={})".format(
                self.conv,
                self.global_pusqnorm,
                self.global_pwsqnorm,
                self.global_pvsqnorm,
                self.global_pzsqnorm,
            )
        )
        ...
        if not self.bundling:
            for dl in dlist:
                ...
                for (ndn, i), xvar in scenario._mpisppy_data.nonant_indices.items():
                    scenario._mpisppy_model.z_foropt[(ndn, i)] = (
                        scenario._mpisppy_model.z[(ndn, i)]
                    )
                    scenario._mpisppy_model.W_foropt[(ndn, i)] = (
                        scenario._mpisppy_model.W[(ndn, i)]
                    )
        else:
            for dl in dlist:
    ...
    def APH_solve_loop(
        ...
        if use_scenarios_not_subproblems:
            ...
            if not self.bundling:
                ...
                phidict = {
                    k: self.phis[self.local_subproblems[k].scen_list[0]]
        ...
        def _dispatch_list(scnt):
            ...
            if self.round_robin_dispatch:
                ...
                sortedbyI = {
                    k: v
                    for k, v in sorted(
                        self.dispatchrecord.items(), key=lambda item: item[1][-1]
                    )
                }
                ...
                for k, v in sortedbyI.items():
                    ...
                    if i >= scnt:
                        return retval
                raise RuntimeError(
                    f"bad scnt={scnt} in _dispatch_list;"
                ...
                tosort = [
                    (
                        k,
                        -max(self.dispatchrecord[k][-1][0], self.shelf_life - 1),
                        phidict[k],
                    )
                    for k in self.dispatchrecord.keys()
                ]
                sortedlist = sorted(
                    tosort, key=lambda element: (element[1], element[2])
                )
        ...
        for dguy in dispatch_list:
            ...
            logging.debug(
                "  in APH solve_loop rank={}, k={}, phi={}".format(
                    self.cylinder_rank, k, p
                )
            )
        ...
        if dtiming:
            ...
            print(
                "\trank=,%d, n=,%d, min=,%4.2f, mean=,%4.2f, max=,%4.2f"
                % (
                    self.global_rank,
    ...
    def _print_conv_detail(self):
        ...
        if pwnorm > 0 and pznorm > 0:
            print(
                f"    scaled U term={punorm / pwnorm}; scaled V term={pvnorm / pznorm}"
            )
        else:
    ...
    def display_details(self, msg):
        ...
        print(
            f"phi={self.global_phi}, nu={self.nu}, tau={self.global_tau} so theta={self.theta}"
        )
        ...
        if self.plot_trace_prefix is not None:
            for k, s in self.local_scenarios.items():
                ...
                with open(f"trace_{k}_{self.conv_trace_filename}", "a") as fil:
                    ...
                    for (ndn, i), xvar in s._mpisppy_data.nonant_indices.items():
                        fil.write(
                            f",{xvar._value},{s._mpisppy_model.z[(ndn,i)]._value},{s._mpisppy_model.W[(ndn,i)]._value}"
                        )
                    fil.write("\n")
    ...
    def APH_iterk(self, spcomm):
        ...
        self.dispatch_frac = (
            self.options["dispatch_frac"] if "dispatch_frac" in self.options else 1
        )
        ...
        ddetail = (
            ...
            and self.options["display_convergence_detail"]
        )
        self.conv = None
        ...
        for self._PHIter in range(1, self.options["PHIterLimit"] + 1):
            ...
            logging.debug(
                "phisum={} after step on {}".format(phisum, self.cylinder_rank)
            )
            ...
            if spcomm is not None and type(spcomm) is not mpi.Intracomm:
                ...
                logging.debug(
                    "post sync_with_spokes on rank {}".format(self.cylinder_rank)
                )
            ...
            if ddetail:
                self.display_details(
                    "pre-solve loop (everything is updated from prev iter)"
                )
            # slight divergence from PH, where mid-iter is before conv
            if have_extensions:
                self.extobject.miditer()

            teeme = ("tee-rank0-solves" in self.options) and (
                self.options["tee-rank0-solves"] and self.cylinder_rank == 0
            )
            ...
            if self.use_lag:
                ...
                for (ndn, i), xvar in scenario._mpisppy_data.nonant_indices.items():
                    ...
                    objfct.expr += (
                        ...
                        * (
                            xvar**2
                            - 2.0 * xvar * scenario._mpisppy_model.z_foropt[(ndn, i)]
                            + scenario._mpisppy_model.z_foropt[(ndn, i)] ** 2
                        )
                    )
                    # W term
                    objfct.expr += (
                        scenario._mpisppy_model.W_on
                        * scenario._mpisppy_model.W_foropt[ndn, i]
                        * xvar
                    )
            else:
                for (ndn, i), xvar in scenario._mpisppy_data.nonant_indices.items():
                    if not scenario._mpisppy_data.has_variable_probability:
                        ...
                        objfct.expr += (
                            ...
                            * (
                                xvar**2
                                - 2.0 * xvar * scenario._mpisppy_model.z[(ndn, i)]
                                + scenario._mpisppy_model.z[(ndn, i)] ** 2
                            )
                        )
                    else:
                        objfct.expr += (
                            scenario._mpisppy_data.prob0_mask[ndn][i]
                            * scenario._mpisppy_model.prox_on
                            * (scenario._mpisppy_model.rho[(ndn, i)] / 2.0)
                            * (
                                xvar**2
...
if __name__ == "__main__":
    ...
    if aph.cylinder_rank == 0:
        print("E[obj] for converged solution (probably NOT non-anticipative)", obj)
