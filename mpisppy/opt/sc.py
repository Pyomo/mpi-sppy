...
def _assert_continuous(m: _BlockData):
    for v in m.component_data_objects(pyo.Var, descend_into=True, active=True):
        if not v.is_continuous():
            raise RuntimeError(
                f"Variable {v} in block {m} is not continuous; The Schur-Complement method only supports continuous problems."
            )


class _SCInterface(
    parapint.interfaces.MPIStochasticSchurComplementInteriorPointInterface
):
    def __init__(
        self,
        ...
        super(_SCInterface, self).__init__(
    ...
    def build_model_for_scenario(
        self, scenario_identifier: str
    ) -> Tuple[_BlockData, Dict[Any, _GeneralVarData]]:
        ...
        m._mpisppy_model.weighted_obj = pyo.Objective(
            expr=m._mpisppy_probability * active_obj.expr, sense=active_obj.sense
        )

        nonant_vars = m._mpisppy_data.nonant_indices
        if len(nonant_vars) != len(self.nonant_vars):
            raise ValueError(
                f"Number of non-anticipative variables is not consistent in scenario {scenario_identifier}."
            )
...
class SchurComplement(SPBase):
    def __init__(
        ...
        self.interface = _SCInterface(
            local_scenario_models=self.local_scenarios,
    ...
    def solve(self):
        ...
        if options.linalg.solver is None:
            options.linalg.solver = parapint.linalg.MPISchurComplementLinearSolver(
                subproblem_solvers={
                    ndx: parapint.linalg.InteriorPointMA27Interface(
                        cntl_options={1: 1e-6}
                    )
                    for ndx in range(len(self.all_scenario_names))
                },
                schur_complement_solver=parapint.linalg.InteriorPointMA27Interface(
                    cntl_options={1: 1e-6}
                ),
            )

        status = parapint.algorithms.ip_solve(interface=self.interface, options=options)
        if status != parapint.algorithms.InteriorPointStatus.optimal:
            raise RuntimeError(
                "Schur-Complement Interior Point algorithm did not converge"
            )
