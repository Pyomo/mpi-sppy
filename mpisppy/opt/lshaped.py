...
class LShapedMethod(spbase.SPBase):
    ...
    def lshaped_algorithm(self, converger=None):
        ...
        for self.iter in range(max_iter):
            ...
            self.mpicomm.Bcast(x_vals, root=self.cylinder_rank0)
            self.mpicomm.Bcast(eta_vals, root=self.cylinder_rank0)
            self.mpicomm.Bcast(outer_bound, root=self.cylinder_rank0)
