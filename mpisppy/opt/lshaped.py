...
class LShapedMethod(spbase.SPBase):
    ...
    def lshaped_algorithm(self, converger=None):
        ...
        """
        if converger:
            converger = converger(self, self.local_rank, self.n_proc)
        max_iter = 30
        ...
        for self.iter in range(max_iter):
            ...
            self.mpicomm.Bcast(x_vals, root=self.local_rank0)
            self.mpicomm.Bcast(eta_vals, root=self.local_rank0)
            self.mpicomm.Bcast(outer_bound, root=self.local_rank0)
