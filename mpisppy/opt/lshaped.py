...
from pyomo.core import SOSConstraint, Constraint, Var
...
class LShapedMethod(spbase.SPBase):
    ...
    def __init__(
        ...
        if "valid_eta_lb" in options:
            ...
            self.valid_eta_lb = {
                scen: (-sys.maxsize - 1) * 1.0 / len(self.all_scenario_names)
    ...
    def _create_root_no_scenarios(self):
        ...
        for var in list(
            root.component_data_objects(Var, active=True, descend_into=True)
        ):
    ...
    def _create_root_with_scenarios(self):
        ...
        if len(ef_scenarios) > 1:
            ...
            def scenario_creator_wrapper(name, **creator_options):
                ...
                if not hasattr(scenario, "_mpisppy_probability"):
                    scenario._mpisppy_probability = 1.0 / len(self.all_scenario_names)
                return scenario
            ...
            if (
                not hasattr(root, "_mpisppy_probability")
                or root._mpisppy_probability == "uniform"
            ):
                root._mpisppy_probability = 1.0 / len(self.all_scenario_names)
        ...
        eta_indx = [
            scenario_name
            for scenario_name in self.all_scenario_names
    ...
    def set_eta_bounds(self):
        if self.compute_eta_bound:
            ...
            this_etas_lb = np.fromiter(
                (self.valid_eta_lb[scen] for scen in self.all_scenario_names),
                float,
                count=len(self.all_scenario_names),
            )
    ...
    def attach_nonant_var_map(self, scenario_name):
        ...
        for var, rvar in zip(
            instance._mpisppy_data.nonant_indices.values(), self.root_vars
        ):
        ...
        instance._mpisppy_model.subproblem_to_root_vars_map = (
    ...
    def create_subproblem(self, scenario_name):
        ...
        if not hasattr(instance, "_mpisppy_probability"):
            instance._mpisppy_probability = 1.0 / self.scenario_count
            ...
            if verbose and self.cylinder_rank == 0:
                if self.iter > 0:
                    print(
                        ...
                        "Time Elapsed:",
                        "%7.2f" % (time.time() - t),
                        "Time Spent on Last Master:",
                        "%7.2f" % t1,
                        "Time Spent Generating Last Cut Set:",
                        "%7.2f" % t2,
                        "Current Objective:",
                        "%7.2f" % m.obj.expr(),
                    )
...
def _get_nonant_ids_EF(instance):
    ...
    for s in snames:
        nonant_ids.update(
            (
                id(v)
                for v in getattr(instance, s)._mpisppy_node_list[0].nonant_vardata_list
            )
        )
