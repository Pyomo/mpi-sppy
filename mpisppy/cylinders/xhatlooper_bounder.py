...
mpisppy.log.setup_logger(
    "mpisppy.cylinders.xhatlooper_bounder", "xhatlp.log", level=logging.CRITICAL
)
    ...
    def main(self):
        ...
        while not self.got_kill_signal():
            if (xh_iter - 1) % 10000 == 0:
                logger.debug(
                    f"   Xhatlooper loop iter={xh_iter} on rank {self.global_rank}"
                )
            ...
            if self.new_nonants:
                ...
                upperbound, srcsname = xhatter.xhat_looper(
                    scen_limit=scen_limit, restore_nonants=True
                )
