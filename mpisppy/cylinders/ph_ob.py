...
class PhOuterBound(mpisppy.cylinders.spoke.OuterBoundSpoke):
    ...
    def ph_ob_prep(self):
        ...
        if (
            "ph_ob_rho_rescale_factors_json" in self.opt.options
            and self.opt.options["ph_ob_rho_rescale_factors_json"] is not None
        ):
        ...
        if "ph_ob_gradient_rho" in self.opt.options:
            assert (
                self.opt.options["ph_ob_gradient_rho"]["cfg"] is not None
            ), "You need to give a cfg to use gradient rho."
            ...
            if "rho_denom" in self.opt.options["ph_ob_gradient_rho"]:
                self.cfg.grad_rho_denom = self.opt.options["ph_ob_gradient_rho"][
                    "rho_denom"
                ]
            self.cfg.grad_cost_file_out = "_temp_grad_cost_file_ph_ob.csv"
            self.cfg.grad_rho_file_out = (
                "_temp_grad_rho_file_ph_ob.csv"  # out??? xxxx tbd
            )
    ...
    def _display_xbar_values(self):
        for sname, scenario in self.opt.local_scenarios.items():
            xbar_list = [
                scenario._mpisppy_model.xbars[ndn_i]._value
                for ndn_i in scenario._mpisppy_data.nonant_indices.keys()
            ]
