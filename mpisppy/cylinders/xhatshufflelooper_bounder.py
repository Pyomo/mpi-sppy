...
mpisppy.log.setup_logger(
    "mpisppy.cylinders.xhatshufflelooper_bounder", "xhatclp.log", level=logging.CRITICAL
)
...
class XhatShuffleInnerBound(spoke.InnerBoundNonantSpoke):
    ...
    def try_scenario_dict(self, xhat_scenario_dict):
        ...
        branching_factors = self.opt.options.get(
            "branching_factors", None
        )  # for stage2ef
        obj = self.xhatter._try_one(
            snamedict,
...
class ScenarioCycler:
    ...
    def _fill_nodescen_dict(self, empty_nodes):
        ...
        while len(empty_nodes) > 0:
            ...
            if (
                filling_idx == self._cycle_idx
                and "ROOT" in self.nodescen_dict
                and self.nodescen_dict["ROOT"] is not None
            ):
                print(self.nodescen_dict)
                raise RuntimeError(
                    "_fill_nodescen_dict looped over every scenario but was not able to find a scen for every nonleaf node."
                )
    ...
    def _begin_normal_epoch(self):
        ...
        self._cur_ROOTscen = (
            self._shuffled_snames[0] if self.best is None else self.best
        )
    ...
    def _iter_scen(self):
        ...
        if old_idx < self._cycle_idx:
            ...
            scens_to_remove = (
                self._shuffled_snames[old_idx:]
                + self._shuffled_snames[: self._cycle_idx]
            )
