...
class Test_gradient_farmer(unittest.TestCase):
    ...
    def test_compute_and_write_grad_rho(self):
        ...
        try:
            ...
            raise RuntimeError(
                "gradient.compute_and_write_grad_rho() did not write a csv file"
            )
...
class Test_find_rho_farmer(unittest.TestCase):
    ...
    def test_w_denom(self):
        ...
        denom = {
            node.name: self.rho_object._w_denom(s0, node)
            for node in s0._mpisppy_node_list
        }
    ...
    def test_prox_denom(self):
        ...
        k0, s0 = list(self.rho_object.ph_object.local_scenarios.items())[0]
        denom = {
            node.name: self.rho_object._prox_denom(s0, node)
            for node in s0._mpisppy_node_list
        }
    ...
    def _run_ph_farmer(self):
        ...
        hub_dict = vanilla.ph_hub(
            *beans,
            ...
            ph_converger=None,
        )  # ??? DLW Oct 2023: is this correct?
        hub_dict["opt_kwargs"]["extension_kwargs"] = {"ext_classes": ext_classes}

        hub_dict["opt_kwargs"]["options"]["gradient_extension_options"] = {
            "cfg": self.cfg
        }
