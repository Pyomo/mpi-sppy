...
class TestFarmerAverageScenarioCreator(unittest.TestCase):
    """Unit tests for the farmer average_scenario_creator."""

    def test_requires_num_scens(self):
        with self.assertRaises(ValueError):
            farmer.average_scenario_creator("Average")

    def test_probability_is_one(self):
        avg = farmer.average_scenario_creator("Average", num_scens=6)
        self.assertEqual(avg._mpisppy_probability, 1.0)

    def test_two_stage_tree(self):
        avg = farmer.average_scenario_creator("Average", num_scens=6)
        self.assertEqual(len(avg._mpisppy_node_list), 1)
        self.assertEqual(avg._mpisppy_node_list[0].name, "ROOT")

    def test_yields_are_mean_of_per_scenario_yields(self):
        num_scens = 6
        avg = farmer.average_scenario_creator("Average", num_scens=num_scens)
        snames = farmer.scenario_names_creator(num_scens)
        per = [farmer._scenario_data(s) for s in snames]
        for crop in avg.CROPS:
            expected = sum(d["Yield"][crop] for d in per) / num_scens
            self.assertAlmostEqual(pyo.value(avg.Yield[crop]), expected)

    def test_seedoffset_is_threaded(self):
        avg0 = farmer.average_scenario_creator("Average", num_scens=6, seedoffset=0)
        avg7 = farmer.average_scenario_creator("Average", num_scens=6, seedoffset=7)
        y0 = {c: pyo.value(avg0.Yield[c]) for c in avg0.CROPS}
        y7 = {c: pyo.value(avg7.Yield[c]) for c in avg7.CROPS}
        self.assertNotEqual(y0, y7)

    def test_deterministic(self):
        a = farmer.average_scenario_creator("Average", num_scens=6)
        b = farmer.average_scenario_creator("Average", num_scens=6)
...
class TestJensensMixinEndToEnd(unittest.TestCase):
    """Light integration: build the average scenario, solve it, pack the cache."""
...
class TestSizesOuterBoundRejected(unittest.TestCase):
    ...
    def test_xhat_path_skips_integer_guard(self):
        ...
        # assert that building the average scenario itself is fine.
        sp = self._spoke()
        avg = sp._jensens_build_avg()
        self.assertTrue(hasattr(avg, "NumProducedSecondStage"))


class TestMissingAverageScenarioCreator(unittest.TestCase):
    ...
    def test_maybe_attach_jensens_installs_dict_when_flag_on(self):
        ...
        self.assertIs(j["average_scenario_creator"], farmer.average_scenario_creator)
        self.assertIs(j["scenario_creator_kwargs"], kwargs)


class TestJensensBuildAvgErrors(unittest.TestCase):
    """_jensens_build_avg rejects ill-shaped average scenarios."""

    def _spoke_with_creator(self, creator):
        jdict = {
            "average_scenario_creator": creator,
            "scenario_creator_kwargs": {},
        }
