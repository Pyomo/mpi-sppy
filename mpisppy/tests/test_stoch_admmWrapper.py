...
class TestStochAdmmWrapper(unittest.TestCase):
    ...
    def _make_admm(self, num_stoch_scens, num_admm_subproblems, verbose=False):
        ...
        admm, all_admm_stoch_subproblem_scenario_names = (
            stoch_distr_admm_cylinders._make_admm(cfg, n_cylinders=1, verbose=verbose)
        )
        return admm
    ...
    def test_assign_variable_probs_error1(self):
        ...
        admm.consensus_vars["Region1"].append(
            (self._slack_name("DC2DC3"), 2)
        )  # The variable is added in the second stage
    ...
    def _extracting_output(self, line):
        ...
        if match:
            ...
            raise RuntimeError(
                "The test is probably not correctly adapted: can't match the format of the line"
            )

    def test_values(self):
        command_line_pairs = [
            (
                f"mpiexec -np 3 python -u -m mpi4py stoch_distr_admm_cylinders.py --num-stoch-scens 10 --num-admm-subproblems 2 --default-rho 10 --solver-name {solver_name} --max-iterations 50 --xhatxbar --lagrangian --rel-gap 0.001 --num-stage 3",
        ...
        for j in range(len(command_line_pairs)):
            if (
                j == 0
            ):  # The first line is executed in the test directory because it has a 3-stage problem. This one does not insure xhatfeasibility but luckily works
                target_directory = "examples/stoch_distr"
            if (
                j != 0
            ):  # The other lines are executed in the real directory because it ensures xhat feasibility
                target_directory = "../../examples/stoch_distr"
            ...
            for i in range(len(result_by_line)):
                if (
                    "EF objective" in result_by_line[-i - 1]
                ):  # should be on last line but we can check
                    decomposed_line = result_by_line[-i - 1].split(": ")
                    objectives["EF objective"] = float(
                        decomposed_line[1]
                    )  # math.ceil(float(decomposed_line[1]))
            try:
                correct_order = (
                    objectives["outer bound"]
                    <= objectives["EF objective"]
                    <= objectives["inner bound"]
                )
            except Exception:
