...
class Test_confint_aircond(unittest.TestCase):
    ...
    def test_ama_creator(self):
        ...
        ama_object = ama.from_module(
            self.refmodelname, cfg=options, use_command_line=False
        )
    ...
    def test_MMW_constructor(self):
        ...
        MMW = MMWci.MMWConfidenceIntervals(
            self.refmodelname,
            options,
            xhat,
            options["num_batches"],
    ...
    def test_indepscens_seqsampling_creator(self):
        ...
        multi_seqsampling.IndepScens_SeqSampling(
            self.refmodelname,
            aircond.xhat_generator_aircond,
            optionsBM,
            stochastic_sampling=False,
    ...
    def test_ama_running(self):
        options, scenario_creator_kwargs = self._get_base_options()
        ama_object = ama.from_module(self.refmodelname, options, use_command_line=False)
        ama_object.run()
        obj = round_pos_sig(ama_object.EF_Obj, 2)
        self.assertEqual(obj, 970.0)
    ...
    def test_gap_estimators(self):
        ...
        sample_options = {"seed": 0, "branching_factors": global_BFs}
    ...
    def test_indepscens_seqsampling_running(self):
        ...
        seq_pb = multi_seqsampling.IndepScens_SeqSampling(
            self.refmodelname,
            aircond.xhat_generator_aircond,
            optionsBM,
            stochastic_sampling=False,
