...
def APL1P_model_creator(seed):
    ...
    model.Availability = pyo.Param(
        model.G, within=pyo.NonNegativeReals, initialize=availability_init
    )
    ...
    model.Investment = pyo.Param(
        model.G, within=pyo.NonNegativeReals, initialize=investment_init
    )
    ...
    model.OperatingCost = pyo.Param(
        model.G, model.DL, within=pyo.NonNegativeReals, initialize=operatingcost_init
    )
    ...
    model.Demand = pyo.Param(
        model.DL, within=pyo.NonNegativeReals, initialize=demand_init
    )

    # Cost of unserved demand
    unserved_cost = 10.0
    model.CostUnservedDemand = pyo.Param(
        model.DL, within=pyo.NonNegativeReals, initialize=unserved_cost
    )
    ...
    def MaximumOperating_rule(model, g):
        return (
            sum(model.OperationLevel[g, dl] for dl in model.DL)
            <= model.Availability[g] * model.CapacityGenerators[g]
        )
    ...
    def SatisfyDemand_rule(model, dl):
        return (
            sum(model.OperationLevel[g, dl] for g in model.G) + model.UnservedDemand[dl]
            >= model.Demand[dl]
        )
    ...
    def ComputeSecondStageCost_rule(model):
        expr = sum(
            model.OperatingCost[g, dl] * model.OperationLevel[g, dl]
            for g in model.G
            for dl in model.DL
        ) + sum(
...
def xhat_generator_apl1p(scenario_names, solver_name="gurobi", solver_options=None):
    ...
    ama = amalgamator.from_module(
        "mpisppy.tests.examples.apl1p", ama_options, use_command_line=False
    )
...
if __name__ == "__main__":
    ...
    apl1p_pb = SeqSampling(
        "mpisppy.tests.examples.apl1p",
