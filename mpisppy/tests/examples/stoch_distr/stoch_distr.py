...
def inter_region_dict_creator(
...
):  # doesn't depend on the scenarios in this example
    ...
    if num_admm_subproblems == 2:
        ...
        inter_region_dict["arcs"] = [
            (("Region1", "DC1"), ("Region2", "DC2")),
            (("Region2", "DC2"), ("Region1", "DC1")),
            (("Region1", "DC1"), ("Region3", "DC3_1")),
            (("Region3", "DC3_1"), ("Region1", "DC1")),
            (("Region2", "DC2"), ("Region3", "DC3_2")),
            (("Region3", "DC3_2"), ("Region2", "DC2")),
        ]
        inter_region_dict["costs"] = {
            (("Region1", "DC1"), ("Region2", "DC2")): 100,
            (("Region2", "DC2"), ("Region1", "DC1")): 50,
            (("Region1", "DC1"), ("Region3", "DC3_1")): 200,
            (("Region3", "DC3_1"), ("Region1", "DC1")): 200,
            (("Region2", "DC2"), ("Region3", "DC3_2")): 200,
            (("Region3", "DC3_2"), ("Region2", "DC2")): 200,
        }
        inter_region_dict["capacities"] = {
            (("Region1", "DC1"), ("Region2", "DC2")): 70,
            (("Region2", "DC2"), ("Region1", "DC1")): 100,
            (("Region1", "DC1"), ("Region3", "DC3_1")): 50,
            (("Region3", "DC3_1"), ("Region1", "DC1")): 50,
            (("Region2", "DC2"), ("Region3", "DC3_2")): 50,
            (("Region3", "DC3_2"), ("Region2", "DC2")): 50,
        }
        ...
        inter_region_dict["arcs"] = [
            ...
            (("Region2", "DC2"), ("Region3", "DC3_2")),
            (("Region3", "DC3_2"), ("Region2", "DC2")),
            (("Region1", "DC1"), ("Region4", "DC4")),
            (("Region4", "DC4"), ("Region1", "DC1")),
            (("Region4", "DC4"), ("Region2", "DC2")),
            (("Region2", "DC2"), ("Region4", "DC4")),
        ]
    inter_region_dict["costs"] = {
        (("Region1", "DC1"), ("Region2", "DC2")): 100,
        (("Region2", "DC2"), ("Region1", "DC1")): 50,
        ...
        (("Region2", "DC2"), ("Region3", "DC3_2")): 200,
        (("Region3", "DC3_2"), ("Region2", "DC2")): 200,
        (("Region1", "DC1"), ("Region4", "DC4")): 30,
        (("Region4", "DC4"), ("Region1", "DC1")): 50,
        (("Region4", "DC4"), ("Region2", "DC2")): 100,
        (("Region2", "DC2"), ("Region4", "DC4")): 70,
    }
    inter_region_dict["capacities"] = {
        (("Region1", "DC1"), ("Region2", "DC2")): 70,
        (("Region2", "DC2"), ("Region1", "DC1")): 100,
        ...
        (("Region2", "DC2"), ("Region3", "DC3_2")): 50,
        (("Region3", "DC3_2"), ("Region2", "DC2")): 50,
        (("Region1", "DC1"), ("Region4", "DC4")): 100,
        (("Region4", "DC4"), ("Region1", "DC1")): 60,
        (("Region4", "DC4"), ("Region2", "DC2")): 20,
        (("Region2", "DC2"), ("Region4", "DC4")): 40,
    }
...
def dummy_nodes_generator(region_dict, inter_region_dict):
    ...
    for arc in inter_region_dict["arcs"]:
        ...
        if region_source == region_dict["name"]:
            ...
            local_dict["flow costs"][(node_source, dummy_node)] = (
                inter_region_dict["costs"][(source, target)] / 2
            )  # should be adapted to model
            local_dict["flow capacities"][(node_source, dummy_node)] = (
                inter_region_dict["capacities"][(source, target)]
            )
            local_dict["dummy nodes source slack bounds"][dummy_node] = (
                inter_region_dict["capacities"][(source, target)]
            )
        ...
        if region_target == local_dict["name"]:
            ...
            local_dict["flow costs"][(dummy_node, node_target)] = (
                inter_region_dict["costs"][(source, target)] / 2
            )  # should be adapted to model
            local_dict["flow capacities"][(dummy_node, node_target)] = (
                inter_region_dict["capacities"][(source, target)]
            )
            local_dict["dummy nodes target slack bounds"][dummy_node] = (
                inter_region_dict["capacities"][(source, target)]
            )
    return local_dict
...
def region_dict_creator(
    admm_subproblem_name,
):  # in this precise example region_dict doesn't depend on the scenario
    ...
    assert _is_partition(
        region_dict["nodes"],
        region_dict["factory nodes"],
        region_dict["buyer nodes"],
        region_dict["distribution center nodes"],
    )
...
def min_cost_distr_problem(
    local_dict, stoch_scenario_name, cfg, demand=None, sense=pyo.minimize
):
    ...
    if demand is not None:
        if demand == "high":
            ...
            raise RuntimeError(
                f"for the 3-stage problem demand should be 'high' or 'low', but it is {demand=}"
            )
    ...
    def slackBounds_rule(model, n):
        if n in local_dict["factory nodes"]:
            ...
            n in local_dict["dummy nodes target"]
        ):  # this slack will respect the opposite flow balance rule
    ...
    model.FirstStageCost = pyo.Expression(
        expr=sum(
            local_dict["production costs"][n] * (local_dict["supply"][n] - model.y[n])
    ...
    if demand is not None:  # 3-stage example
        model.SecondStageCost = pyo.Expression(
            expr=sum(-810 * model.export[n] for n in local_dict["buyer nodes"])
        )

    model.LastStageCost = pyo.Expression(
        expr=sum(
    ...
    if demand is not None:  # 3-stage example
        model.MinCost = pyo.Objective(
            expr=model.FirstStageCost + model.SecondStageCost + model.LastStageCost,
            sense=sense,
        )
    else:  # 2-stage example
        model.MinCost = pyo.Objective(
            expr=model.FirstStageCost + model.LastStageCost, sense=sense
        )
    ...
    def FlowBalance_rule(m, n):
        ...
        if n in local_dict["dummy nodes target"]:
            ...
            return sum(m.flow[a] for a in arcsout[n]) - sum(
                m.flow[a] for a in arcsin[n]
            ) == (local_dict["supply"][n] - m.y[n]) * min(
                1, max(0, 1 - np.random.normal(cfg.spm, cfg.cv) / 100)
            )  # We add the loss
        elif (
            n in local_dict["buyer nodes"] and demand is not None
        ):  # for a 3-stage scenario the export changes the flow balance
            return (
                sum(m.flow[a] for a in arcsout[n])
                - sum(m.flow[a] for a in arcsin[n])
                + m.y[n]
                == local_dict["supply"][n] - m.export[n]
            )
        else:
            return (
                sum(m.flow[a] for a in arcsout[n])
...
def MakeNodesforScen(model, BFs, scennum, local_dict):
    ...
    retval = [
        ...
        scenario_tree.ScenarioNode(
            ...
            model.FirstStageCost,
            [
                model.export[n] for n in local_dict["buyer nodes"]
...
def branching_factors_creator(num_stoch_scens, num_stage):
    ...
    if num_stage == 3:
        ...
        assert (
            num_stoch_scens % 2 == 0
        ), "there should be an even number of stochastic scenarios for this 3 stage problem, but it is odd"
        BFs = [num_stoch_scens // 2, 2]
    elif num_stage == 2:
        BFs = None  # No need to use branching factors (or even to define them with two stage problems)
    else:
        raise RuntimeError(
            f"the example is only made for 2 or 3-stage problems, but {num_stage} were given"
        )
    return BFs
...
def scenario_creator(admm_stoch_subproblem_scenario_name, **kwargs):
    ...
    admm_subproblem_name, stoch_scenario_name = (
        split_admm_stoch_subproblem_scenario_name(admm_stoch_subproblem_scenario_name)
    )
    ...
    if cfg.num_stage == 2:
        ...
        if stoch_scenario_name.endswith("high"):
            ...
            raise RuntimeError(
                f"the stochastic scenario name should end with 'high' or 'low', but it is {stoch_scenario_name}"
            )
        model = min_cost_distr_problem(
            local_dict, stoch_scenario_name, cfg, demand=demand
        )
        scennum = _scenario_number(stoch_scenario_name, demand=demand)
...
def consensus_vars_creator(
    admm_subproblem_names, stoch_scenario_name, kwargs, num_stage=2
):
    ...
    for admm_subproblem_name in admm_subproblem_names:
        admm_stoch_subproblem_scenario_name = combining_names(
            admm_subproblem_name, stoch_scenario_name
        )
...
def stoch_scenario_names_creator(num_stoch_scens, num_stage=2):
    ...
    if num_stage == 3:
        return [
            f"StochasticScenario{i+1}_{demand}"
            for i in range(num_stoch_scens // 2)
