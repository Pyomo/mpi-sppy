...
def _StageModel_creator(time, demand, last_stage, **kwargs):
    ...
    # Variables
    model.RegularProd = pyo.Var(domain=pyo.NonNegativeReals, bounds=(0, model.bigM))

    model.OvertimeProd = pyo.Var(domain=pyo.NonNegativeReals, bounds=(0, model.bigM))

    model.Inventory = pyo.Var(domain=pyo.Reals, bounds=(-model.bigM, model.bigM))
    ...
    model.negInventory = pyo.Var(
        domain=pyo.NonNegativeReals, initialize=0.0, bounds=(0, model.bigM)
    )
    model.posInventory = pyo.Var(
        domain=pyo.NonNegativeReals, initialize=0.0, bounds=(0, model.bigM)
    )
    model.doleInventory = pyo.Constraint(
        expr=model.Inventory == model.posInventory - model.negInventory
    )

    # create the inventory cost expression
    if not last_stage:
        model.LinInvenCostExpr = pyo.Expression(
            expr=model.InventoryCost * model.posInventory
            + model.NegInventoryCost * model.negInventory
        )
    else:
        assert (
            model.LastInventoryCost < 0
        ), f"last stage inven cost: {model.LastInventoryCost}"
        model.LinInvenCostExpr = pyo.Expression(
            expr=model.LastInventoryCost * model.posInventory
            + model.NegInventoryCost * model.negInventory
        )

    if model.QuadShortCoeff > 0 and not last_stage:
        model.InvenCostExpr = pyo.Expression(
            expr=model.LinInvenCostExpr
...
def aircond_model_creator(demands, **kwargs):
    ...
    if model.T[-1] > model.max_T:
        raise RuntimeError(f"The number of stages exceeds {model.max_T}")
    ...
    for t in model.T:
        ...
        model.stage_models[t] = _StageModel_creator(
            t, demands[t - 1], last_stage=last_stage, **kwargs
        )
    ...
    def material_balance_rule(m, t):
        if t == 1:
            return (
                m.BeginInventory
                + m.stage_models[t].RegularProd
                + m.stage_models[t].OvertimeProd
                - m.stage_models[t].Inventory
                == m.stage_models[t].Demand
            )
        else:
            return (
                m.stage_models[t - 1].Inventory
                + m.stage_models[t].RegularProd
                + m.stage_models[t].OvertimeProd
                - m.stage_models[t].Inventory
                == m.stage_models[t].Demand
    ...
    model.TotalCostObjective = pyo.Objective(rule=total_cost_rule, sense=pyo.minimize)
    ...
    **scenario_creator_kwargs,
    ...
    if given_scenario is None:
        if stage == 1:
            ...
            raise RuntimeError(
                "sample_tree_scen_creator for aircond needs a 'given_scenario' argument if the starting stage is greater than 1"
            )
    else:
        past_demands = [
            given_scenario.stage_models[t].Demand
            for t in given_scenario.T
            if t <= stage
        ]
    ...
    future_demands, nodenames = _demands_creator(
        sname, sample_branching_factors, root_name="ROOT" + "_0" * (stage - 1), **kwargs
    )

    demands = (
        past_demands + future_demands[1:]
    )  # The demand at the starting stage is in both past and future demands
    ...
    # Constructing the nodes used by the scenario
    model._mpisppy_node_list = MakeNodesforScen(
        model, nodenames, sample_branching_factors, starting_stage=stage
    )
...
def inparser_adder(cfg):
    ...
    _doone(
        "mu_dev", "average deviation of demand between two periods", argname="mu-dev"
    )
    _doone(
        "sigma_dev",
        "standard deviation of deviation of demand between two periods",
        argname="sigma-dev",
    )
    _doone(
        "start_ups", "Include start-up costs in model, resulting in a MPI (default {d})"
    )
    _doone(
        "StartUpCost",
        helptext="Cost if production in a period is non-zero and start-up is True",
    )
    ...
    _doone(
        "LastInventoryCost",
        helptext="Inventory `cost` (should be negative) in last period)",
    )
    ...
    _doone(
        "NegInventoryCost",
        helptext="Linear coefficient for backorders (should be negative; not used in last stage)",
    )
    _doone(
        "QuadShortCoeff",
        helptext="Coefficient for backorders squared (should be nonnegative; not used in last stage)",
    )
...
def kw_creator(cfg, optionsin=None):
    ...
    if kwargs["start_ups"] is None:
        raise ValueError(
            f"kw_creator called, but no value given for start_ups, options={options}"
        )
    if kwargs["start_seed"] is None:
        raise ValueError(
            f"kw_creator called, but no value given for start_seed, options={options}"
        )
...
def xhat_generator_aircond(
    ...
    assert (
        branching_factors is not None
    ), "branching factors must be supplied to xhat_generator_aircond"
    assert (
        solver_name is not None
    ), "solver_name must be supplied to xhat_generator_aircond"
    ...
    ama = amalgamator.from_module(
        "mpisppy.tests.examples.aircond", cfg, use_command_line=False
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--solver-name", help="solver name", default=None)
    parser.add_argument(
        "--branching-factors",
    ...
    parser.add_argument(
        "--save-xhat", help="Toggles file save", dest="save_xhat", action="store_true"
    )
    ...
    # We use from_module to build easily an Amalgamator object
    t0 = time.time()
    ama = amalgamator.from_module(refmodel, ama_options, use_command_line=False)
    ama.run()
