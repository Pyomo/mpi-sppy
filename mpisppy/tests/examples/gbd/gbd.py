...
def GBD_model_creator(seed):
    ...
    demand_cumprobs = (
        np.flip(np.cumsum(np.flip(demand_probs[0]))),
    ...
    model.passengerDemand = pyo.Param(
        model.route, within=pyo.NonNegativeReals, initialize=demands_init
    )
    ...
    def AircraftSupply_rule(m, a):
        return (
            sum(model.x[a, j] for j in model.route) + model.aircraftSlack[a]
            == model.numAircrafts[a]
        )

    model.SatisfyAircraftConstraints = pyo.Constraint(
        model.aircraftType, rule=AircraftSupply_rule
    )

    # route demand constraint
    def RouteDemand_rule(m, r):
        return (
            sum(model.p[i + 1, r + 1] * model.x[i, r] for i in model.aircraftType)
            + model.p[5, r + 1]
            * (model.passengerSlack_pos[r] - model.passengerSlack_neg[r])
            == model.passengerDemand[r]
        )
    ...
    def ComputeOperatingCosts(m):
        return sum(
            model.c[i + 1, j + 1] * model.x[i, j]
            for i in model.aircraftType
            for j in model.route
        )
...
def xhat_generator_gbd(scenario_names, solver_name="gurobi", solver_options=None):
    ...
    ama = amalgamator.from_module(
        "mpisppy.tests.examples.gbd.gbd", ama_options, use_command_line=False
    )
...
if __name__ == "__main__":
    ...
    gbd_pb = SeqSampling(
        "mpisppy.tests.examples.gbd.gbd",
        xhat_generator_gbd,
        optionsFSP,
