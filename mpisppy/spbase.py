...
class SPBase:
    ...
    def _verify_nonant_lengths(self):
        ...
        for k, s in self.local_scenarios.items():
            ...
            for node in s._mpisppy_node_list:
                ...
                if ndn not in local_node_nonant_lengths:
                    ...
                    raise RuntimeError(
                        f"Tree node {ndn} has scenarios with different numbers of non-anticipative "
        ...
        for ndn, val in local_node_nonant_lengths.items():
            ...
            self.comms[ndn].Allreduce(
                [local_val, MPI.INT], [max_val, MPI.INT], op=MPI.MAX
            )
    ...
    def _check_nodenames(self):
        for ndn in self.all_nodenames:
            if ndn != "ROOT" and sputils.parent_ndn(ndn) not in self.all_nodenames:
                raise RuntimeError(
                    f"all_nodenames is inconsistent:"
    ...
    def _create_scenarios(self, scenario_creator_kwargs):
        ...
        for sname in self.local_scenario_names:
            ...
            if self.multistage:
                ...
                if (s._mpisppy_node_list[stmax].name) + "_0" not in self.all_nodenames:
                    raise RuntimeError(
                        "The leaf node associated with this scenario is not on all_nodenames"
        ...
        if self.options.get("display_timing", False):
            all_instance_creation_times = self.mpicomm.gather(local_ict, root=0)
            if self.cylinder_rank == 0:
                aict = [ict for l_ict in all_instance_creation_times for ict in l_ict]
                print("Scenario instance creation times:")
                print(
                    f"\tmin={np.min(aict):4.2f} mean={np.mean(aict):4.2f} max={np.max(aict):4.2f}"
                )
    ...
    def _create_communicators(self):
        ...
        for nodename in nonleafnodes:
            if nodename not in self.all_nodenames:
                raise RuntimeError(
                    f"Tree node '{nodename}' not in all_nodenames list {self.all_nodenames} for {self.global_rank=}"
                )
        ...
        for nodename in self.all_nodenames:
            if nodename == "ROOT":
                ...
                self.comms[nodename] = self.mpicomm.Split(
                    color=nodenumber, key=self.cylinder_rank
                )
            ...
            for sname, rank in scenario_names_to_comm_rank.items():
                if sname in self.local_scenarios:
                    if rank != comm.Get_rank():
                        raise RuntimeError(
                            f"For the node {nodename}, the scenario {sname} has the rank {rank} from scenario_names_to_rank and {comm.Get_rank()} from its comm."
                        )
    ...
    def _compute_unconditional_node_probabilities(self):
        ...
        for k, s in self.local_scenarios.items():
            ...
            for parent, child in zip(
                s._mpisppy_node_list[:-1], s._mpisppy_node_list[1:]
            ):
            ...
            if not hasattr(s._mpisppy_data, "prob_coeff"):
                ...
                for node in s._mpisppy_node_list:
                    s._mpisppy_data.prob_coeff[node.name] = (
    ...
    def _use_variable_probability_setter(self, verbose=False):
        ...
        didit = 0
        skipped = 0
        variable_probability_kwargs = (
            self.options["variable_probability_kwargs"]
            if "variable_probability_kwargs" in self.options
            else dict()
        )
        sum_probs = {}  # indexed by (ndn,i) - maps to sum of probs for that variable
        for sname, s in self.local_scenarios.items():
            variable_probability = self.variable_probability(
                s, **variable_probability_kwargs
            )
            ...
            for vid, prob in variable_probability:
                ...
                if type(s._mpisppy_data.prob_coeff[ndn]) is float:  # not yet a vector
                    ...
                    s._mpisppy_data.prob_coeff[ndn] = np.full(
                        s._mpisppy_data.nlens[ndn], defprob, dtype="d"
                    )
                    s._mpisppy_data.prob0_mask[ndn] = np.ones(
                        s._mpisppy_data.nlens[ndn], dtype="d"
                    )
    ...
    def is_zero_prob(self, scenario_model, var):
        ...
        if isinstance(_mpisppy_data.prob_coeff[ndn], np.ndarray):
            return float(_mpisppy_data.prob_coeff[ndn][i]) == 0.0
        else:
            return False
        ...
        for k, s in self.local_scenarios.items():
            ...
            for node in s._mpisppy_node_list:
                ...
                if ndn not in checked_nodes:
                    if not np.allclose(global_concats[ndn], 1.0, atol=tol):
                        notclose = ~np.isclose(global_concats[ndn], 1.0, atol=tol)
                        indices = np.nonzero(notclose)[0]
                        bad_vars = [
                            s._mpisppy_data.nonant_indices[ndn, idx].name
                            for idx in indices
                        ]
                        badprobs = [global_concats[ndn][idx] for idx in indices]
                        raise RuntimeError(
                            f"Node {ndn}, variables {bad_vars} have respective"
    ...
    def _look_and_leap(self):
        for sname, scenario in self.local_scenarios.items():
            ...
            pspec = (
                scenario._mpisppy_probability
                if hasattr(scenario, "_mpisppy_probability")
                else None
            )
            if pspec is None or pspec == "uniform":
                ...
                if self.cylinder_rank == 0 and pspec is None:
                    print(
                        f"Did not find _mpisppy_probability, assuming uniform probability {prob} (avoid this message by assigning a probability or the string 'uniform' to _mpisppy_probability on the scenario model object)"
                    )
    ...
    def _options_check(self, required_options, given_options):
        ...
        """
        missing = [
            option for option in required_options if given_options.get(option) is None
        ]
        if missing:
            raise ValueError(
                f"Missing the following required options: {', '.join(missing)}"
            )
    ...
    def gather_var_values_to_rank0(self, get_zero_prob_values=False):
        ...
        if self.cylinder_rank == 0:
            result = {key: value for dic in result for (key, value) in dic.items()}
            return result
    ...
    def report_var_values_at_rank0(self, header="", print_zero_prob_values=False):
        ...
        var_values = self.gather_var_values_to_rank0(
            get_zero_prob_values=print_zero_prob_values
        )
        ...
        if self.cylinder_rank == 0:
            ...
            for this_scenario in scenario_names:
                print(
                    "{0: ^{width}s} ".format(this_scenario, width=value_field_len),
                    end="",
                )
            print("")
            ...
            for this_var in variable_names:
                print(
                    "{0: <{width}} | ".format(this_var, width=max_variable_name_len),
                    end="",
                )
                for this_scenario in scenario_names:
                    if (this_scenario, this_var) not in var_values:
                        print(
                            "{0: ^{width}s}".format("-", width=value_field_len), end=""
                        )
                    else:
                        this_var_value = var_values[this_scenario, this_var]
                        if (this_var_value is None) and (not print_zero_prob_values):
                            print(
                                "{0: ^{width}s}".format("-", width=value_field_len),
                                end="",
                            )
                        else:
                            print(
                                "{0: {width}.4f}".format(
                                    this_var_value, width=value_field_len
                                ),
                                end="",
                            )
                        print(" ", end="")
    ...
    def write_first_stage_solution(
        self, file_name, first_stage_solution_writer=sputils.first_stage_nonant_writer
    ):
        ...
        if self.cylinder_rank == 0:
            ...
            first_stage_solution_writer(
                file_name, representative_scenario, self.bundling
            )

    def write_tree_solution(
        self,
        directory_name,
        ...
        for scenario_name, scenario in self.local_scenarios.items():
            scenario_tree_solution_writer(
                directory_name, scenario_name, scenario, self.bundling
            )
