...
class SampleSubtree:
    ...
    def __init__(
        ...
        for attr in attrs:
            if not hasattr(self.refmodel, attr):
                raise RuntimeError(
                    f"The construction of a sample subtree failed because the reference model has no {attr} function."
                )
        self.xhats = xhats
    ...
    def _sample_creator(self, sname, **scenario_creator_kwargs):
        ...
        s = self.refmodel.sample_tree_scen_creator(
            sname,
        ...
        for k, ndn in enumerate(self.fixed_nodes):
            ...
            if len(self.xhats[k]) != nlens[ndn]:
                raise RuntimeError(
                    "xhats does not have the right size "
    ...
    def _create_amalgamator(self):
        ...
        cfg.quick_assign(
            "EF-mstage", domain=str, value=len(self.original_branching_factors) > 1
        )
        cfg.quick_assign(
            "EF-2stage", domain=str, value=len(self.original_branching_factors) <= 1
        )
        cfg.quick_assign("EF_solver_name", domain=str, value=self.solver_name)
        if self.solver_options is not None:
            cfg.add_and_assign(
                "solver_options", "solver options dict", dict, None, self.solver_options
            )
        ...
        if "branching_factors" not in cfg:
            ...
            """
        scen_names = self.refmodel.scenario_names_creator(
            self.numscens, start=self.seed
        )
        denouement = (
            self.refmodel.scenario_denouement
            if hasattr(self.refmodel, "scenario_denouement")
            else None
        )

        self.ama = amalgamator.Amalgamator(
            cfg,
            scen_names,
    ...
    def run(self):
        ...
        self.xhat_at_stage = [
            self.ef.ref_vars[(pseudo_root, i)].value
            for i in range(self.ef._nlens[pseudo_root])
        ]


def _feasible_solution(
    mname,
    scenario,
    xhat_one,
    branching_factors,
...
if __name__ == "__main__":
    ...
    cfg.add_and_assign(
        "EF_mstage",
        description="EF mstage (vs 2stage)",
        domain=bool,
        default=None,
        value=True,
    )
    ...
    xhats, seed = _feasible_solution(
        mname, scenario, xhat_one, branching_factors, seed, cfg, solver_name=solver_name
    )
    print(xhats)
