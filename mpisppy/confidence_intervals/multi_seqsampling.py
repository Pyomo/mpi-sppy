...
class IndepScens_SeqSampling(SeqSampling):
    ...
    def run(self, maxit=200):
        ...
        xhat_k = self.xhat_generator(
            xhat_scenario_names, solver_options=self.solver_options, **xgo
        )
    ...
    def _gap_estimators_with_independent_scenarios(
        self, xhat_k, nk, estimator_scenario_names, scenario_denouement
    ):
        ...
        cfg.quick_assign(
            "branching_factors", pyofig.ListOf(int), pseudo_branching_factors
        )
        ...
        local_scenarios = {
            sname: getattr(ama.ef, sname) for sname in ama.ef._ef_scenario_names
        }
        ...
        Gk = ciutils.correcting_numeric(
            G, cfg, objfct=obj_at_xhat, relative_error=use_relative_error
        )
...
if __name__ == "__main__":
    ...
    aircondpb = IndepScens_SeqSampling(
        "mpisppy.tests.examples.aircond",
