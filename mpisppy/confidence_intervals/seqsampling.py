...
def is_needed(cfg, needed_things, message=""):
    ...
    if len(absent) > 0:
        raise RuntimeError(
            "Some options are missing from this list of required options:\n"
            f"{needed_things}\n"
            f"missing: {absent}\n"
            f"{message}"
        )
    ...
    for i, v in optional_things.items():
        if i not in cfg:
            ...
            if isinstance(v, str):
                ...
                raise RuntimeError(
                    f"add_options cannot process type {type(v)} for option {i}:{v}"
                )


def xhat_generator_farmer(
    scenario_names, solver_name=None, solver_options=None, crops_multiplier=1
):
    ...
    ama = amalgamator.from_module(
        "mpisppy.tests.examples.farmer", cfg, use_command_line=False
    )
...
class SeqSampling:
    ...
    def __init__(
        ...
        if not isinstance(cfg, config.Config):
            raise RuntimeError(
                f"SeqSampling bad cfg type={type(cfg)}; should be Config"
            )
        ...
        if self.stopping_criterion == "BM":
            ...
            if not self.stochastic_sampling:
                ...
                optional_things = {
                    "BPL_c1": 2,
                    "functions_dict": {"growth_function": (lambda x: x - 1)},
                }
        ...
        sroot, self.solver_name, self.solver_options = solver_spec.solver_specification(
            cfg, ["EF", ""]
        )
        ...
        if self.solving_type in two_stage_types:
            ...
            raise RuntimeError(
                f"The solving_type {self.solving_type} is not supported. "
        ...
        if q is None:
            ...
            if hasattr(self, "c"):
                ...
                if confidence_level is None:
                    raise RuntimeError(
                        "We need the confidence level to compute the constant cp"
                    )
                j = np.arange(1, 1000)
                s = sum(np.power(j, -p * np.log(j)))
                c = max(
                    1, 2 * np.log(s / (np.sqrt(2 * np.pi) * (1 - confidence_level)))
                )
            ...
            if hasattr(self, "c"):
                ...
                if confidence_level is None:
                    RuntimeError(
                        "We need the confidence level to compute the constant c_pq"
                    )
                j = np.arange(1, 1000)
    ...
    def bpl_fsp_sampsize(self, k, G, s, nk_m1):
        ...
        return int(
            np.ceil(
                self.BPL_c0 + self.BPL_c1 * self.functions_dict["growth_function"](k)
            )
        )
    ...
    def run(self, maxit=200):
        ...
        if self.multistage:
            raise RuntimeWarning(
                "Multistage sequential sampling can be done "
        ...
        if self.stopping_criterion == "BM":
            ...
            self.c = max(
                1, 2 * np.log(s / (np.sqrt(2 * np.pi) * (1 - self.confidence_level)))
            )
        ...
        if self.multistage:
            xhat_branching_factors = ciutils.scalable_branching_factors(
                mult * lower_bound_k, self.cfg["branching_factors"]
            )
            mk = np.prod(xhat_branching_factors)
            self.xhat_gen_kwargs["start_seed"] = (
                self.SeedCount
            )  # TODO: Maybe find a better way to manage seed
        ...
        if self.multistage:
            ...
            gap_branching_factors = ciutils.scalable_branching_factors(
                lower_bound_k, self.cfg["branching_factors"]
            )
            nk = np.prod(gap_branching_factors)
            estimator_scenario_names = refmodel.scenario_names_creator(nk)
            sample_options = {
                "branching_factors": gap_branching_factors,
                "seed": self.SeedCount,
            }
        else:
            nk = self.ArRP * int(np.ceil(lower_bound_k / self.ArRP))
            estimator_scenario_names = refmodel.scenario_names_creator(
        ...
        scenario_denouement = (
            refmodel.scenario_denouement
            if hasattr(refmodel, "scenario_denouement")
            else None
        )
        ...
        while self.stop_criterion(Gk, sk, nk) and k < maxit:
            ...
            if self.multistage:
                xhat_branching_factors = ciutils.scalable_branching_factors(
                    mult * lower_bound_k, lcfg["branching_factors"]
                )
                mk = np.prod(xhat_branching_factors)
                self.xhat_gen_kwargs["start_seed"] = (
                    self.SeedCount
                ...
                if k % self.kf_xhat == 0:
                    ...
                    xhat_scenario_names = refmodel.scenario_names_creator(
                        int(mult * nk), start=self.ScenCount
                    )
                    self.ScenCount += mk
                else:
                    # We reuse the previous scenarios
                    xhat_scenario_names += refmodel.scenario_names_creator(
                        mult * (nk - nk_m1), start=self.ScenCount
                    )
            ...
            if self.multistage:
                ...
                gap_branching_factors = ciutils.scalable_branching_factors(
                    lower_bound_k, lcfg["branching_factors"]
                )
                nk = np.prod(gap_branching_factors)
                estimator_scenario_names = refmodel.scenario_names_creator(nk)
                sample_options = {
        ...
        CI = [0, upper_bound]
        global_toc(
            f"G={Gk} sk={sk}; xhat has been computed with {nk*mult} observations."
        )
        return {
            "T": T,
...
if __name__ == "__main__":
    ...
    optionsBM.quick_assign(
        "stopping", str, "BM"
    )  # TBD use this and drop stopping_criterion from the constructor
    ...
    optionsFSP.quick_assign(
        "ArRP", int, 2
    )  # this must be 1 for any multi-stage problems
