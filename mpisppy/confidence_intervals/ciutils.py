...
def branching_factors_from_numscens(numscens, num_stages):
    ...
    if num_stages == 2:
        ...
        for i in range(2 ** (num_stages - 1)):
            ...
            if (
                sum(prime_fact.values()) >= num_stages - 1
            ):  # Checking that we have enough factors
                branching_factors = [0] * (num_stages - 1)
                fact_list = [
                    factor for (factor, mult) in prime_fact.items() for i in range(mult)
                ]
                for k in range(num_stages - 1):
                    branching_factors[k] = np.prod(
                        [
                            fact_list[(num_stages - 1) * i + k]
                            for i in range(1 + len(fact_list) // (num_stages - 1))
                            if (num_stages - 1) * i + k < len(fact_list)
                        ]
                    )
...
def scalable_branching_factors(numscens, ref_branching_factors):
    ...
    new_branching_factors = np.maximum(
        np.floor(np.array(ref_branching_factors) * mult_coef), 1.0
    )
    i = 0
...
def is_sorted(nodelist):
    ...
    for t, node in enumerate(nodelist):
        if (t + 1 != node.stage) or (node.parent_name != parent):
            raise RuntimeError(
                "The node list is not well-constructed"
...
def _fct_check(module, fct):
    if not hasattr(module, fct):
        raise RuntimeError(
            f"pyomo_opt_sense needs the module to have the {fct} function"
        )
...
def correcting_numeric(G, cfg, relative_error=True, threshold=1e-4, objfct=None):
    ...
    if objfct is None:
        raise RuntimeError(
            "We need a value of the objective function to remove numerically small G"
        )
...
def gap_estimators(
    ...
    if ArRP > 1:  # Special case : ArRP, G and s are pooled from r>1 estimators.
        ...
        if n % ArRP != 0:
            raise RuntimeWarning(
                "You put as an input a number of scenarios"
    ...
    if is_multi:
        ...
        solver_options_str = sputils.option_dict_to_string(
            solver_options
        )  # cfg need str
