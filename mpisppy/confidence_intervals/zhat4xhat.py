...
def evaluate_sample_trees(xhat_one, num_samples, cfg, InitSeed=0, model_module=None):
    ...
    for j in range(num_samples):  # number of sample trees to create
        ...
        xhats, seed = sample_tree.walking_tree_xhats(
            mname,
            local_scenarios,
            xhat_one,
            bfs,
...
def run_samples(cfg, model_module):
    ...
    zhats, seed = evaluate_sample_trees(
        xhat_one, num_samples, cfg, InitSeed=0, model_module=model_module
    )
...
def _main_body(model_module, cfg):
    ...
    lcfg.add_and_assign(
        "EF_solver_options", "solver options dict", dict, None, solver_options_dict
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "model_module_name",
        help="amalgamator compatible module (often read from argv)",
        type=str,
    )
    cfg.add_to_config(
        "model_module_name",
