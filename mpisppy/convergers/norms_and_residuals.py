...
def scaled_primal_metric(PHB):
    ...
    for s in PHB.local_scenarios.values():
        for node in s._mpisppy_node_list:
            ...
            xbars = np.fromiter(
                (s._mpisppy_model.xbars[ndn, i]._value for i in range(nlen)), dtype="d"
            )
            nonants_array = np.fromiter(
                (v._value for v in node.nonant_vardata_list), dtype="d", count=nlen
            )
            scaled_norm = np.divide(
                np.abs(nonants_array - xbars), xbars, out=np.zeros_like(xbars)
            )
            if not s._mpisppy_data.has_variable_probability:
                local_sum[0] += s._mpisppy_data.prob_coeff[ndn] * np.sum(
                    np.multiply(scaled_norm, scaled_norm)
                )
            else:
                # rarely-used overwrite in the event of variable probability
                prob_array = np.fromiter(
                    (
                        s._mpisppy_data.prob_coeff[ndn_i[0]][ndn_i[1]]
...
def scaled_dual_metric(PHB, w_cache, curr_iter):
    ...
    for sname, s in PHB.local_scenarios.items():
        ...
        scaled_norm = np.divide(
            np.abs(current_w - prev_w), np.abs(current_w), out=np.zeros_like(current_w)
        )
        for node in s._mpisppy_node_list:
            ndn = node.name
            nlen = s._mpisppy_data.nlens[ndn]
