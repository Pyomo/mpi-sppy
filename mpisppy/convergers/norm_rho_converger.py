...
class NormRhoConverger(mpisppy.convergers.converger.Converger):
    def __init__(self, ph):
        if (
            "norm_rho_converger_options" in ph.options
            and "verbose" in ph.options["norm_rho_converger_options"]
            and ph.options["norm_rho_converger_options"]["verbose"]
        ):
            self._verbose = True
        else:
            self._verbose = False
    ...
    def _compute_rho_norm(self, ph):
        ...
        local_rho_norm[0] = sum(
            s._mpisppy_probability
            * sum(rho._value for rho in s._mpisppy_model.rho.values())
    ...
    def is_converged(self):
        ...
        if (
            not hasattr(self.ph, "_mpisppy_norm_rho_update_inuse")
        ...
        if self._verbose and self.ph.cylinder_rank == 0:
            ...
            if ret_val:
                ...
                print(
                    "Adaptive rho convergence check failed "
