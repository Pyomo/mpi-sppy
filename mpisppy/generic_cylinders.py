...
if __name__ == "__main__":
    ...
    if cfg.pickle_bundles_dir is not None:
        ...
        if _mmw_requested(cfg) and cfg.get("mmw_xhat_input_file_name") is not None:
            do_mmw(fname, cfg)
    else:
        wheel = do_decomp(module, cfg, scenario_creator, scenario_creator_kwargs, scenario_denouement, bundle_wrapper=bundle_wrapper)
        if _mmw_requested(cfg):
