...
def _name_lists(module, cfg):
    ...
    if cfg.get("branching_factors") is not None:
        ...
        assert (
            not cfg.xhatshuffle or cfg.get("stage2EFsolvern") is not None
...
# ==========
def _do_decomp(
    module, cfg, scenario_creator, scenario_creator_kwargs, scenario_denouement
):
    ...
    if cfg.fixer:  # cfg_vanilla takes care of the fixer_tol?
        assert hasattr(
            module, "id_fix_list_fct"
        ), "id_fix_list_fct required for --fixer"
    ...
    if cfg.primal_dual_converger:
        hub_dict["opt_kwargs"]["options"]["primal_dual_converger_options"] = {
            "verbose": True,
...
def _model_fname():
    def _bad_news():
        raise RuntimeError(
            "Unable to parse module name from first argument"
    ...
    _len_check(1)
    assert (
        sys.argv[1][:13] == "--module-name"
    ), f"The first command argument must start with'--module-name' but you gave {sys.argv[1]}"
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        ...
        print(
            "usage, e.g.: python -m mpi4py ../../mpisppy/generic_cylinders.py --module-name farmer --help"
        )
        quit()
