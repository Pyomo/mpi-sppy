...
class GAMS_guest():
    ...
    def scenario_creator(self, scenario_name, **kwargs):
        ...
        """
        assert kwargs
        assert "nonants_name_pairs" in kwargs
        assert "new_file_names" in kwargs

        nonants_name_pairs = kwargs["nonants_name_pairs"]

        ws = gams.GamsWorkspace(working_directory=this_dir, system_directory=gamspy_base_dir)

        new_file_name = kwargs["new_file_names"]
...
def kw_creator(self, cfg):
    ...
    kwargs = self.model_module.kw_creator(cfg)
    kwargs["cfg"] = cfg
    return kwargs
...
def create_ph_model(original_file, nonants_name_pairs):
    ...
    lines[insert_position + 1] = model_line[:-4] + model_line_text + ", objective_ph_def" + model_line[-4:] #add + ", lo_def" + ", up_def" 
