...
class WheelSpinner:
    ...
    def run(self, comm_world=None):
        ...
        global_toc(
            f"Spoke {sp_class.__name__} finalized",
    ...
    def write_first_stage_solution(
        self, solution_file_name, first_stage_solution_writer=first_stage_nonant_writer
    ):
        ...
        if not self._ran:
            raise RuntimeError(
                "Need to call WheelSpinner.run() before querying solutions."
            )
        winner = self._determine_innerbound_winner()
        if winner:
            self.spcomm.opt.write_first_stage_solution(
                solution_file_name, first_stage_solution_writer
            )
    ...
    def write_tree_solution(
        ...
        winner = self._determine_innerbound_winner()
        if winner:
            self.spcomm.opt.write_tree_solution(
                solution_directory_name, scenario_tree_solution_writer
            )
