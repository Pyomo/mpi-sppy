...
# Make it all go
from mpisppy.utils.sputils import spin_the_wheel, create_EF
# the problem
...
def main():
    ...
    hub_dict = {
        ...
        "opt_kwargs": {
            ...
            'scenario_denouement': None,  # scenario_denouement,
            "cb_data": cb_data,
    ...
    if "hub_class" in opt_dict:  # we are hub rank
        if spcomm.opt.rank == spcomm.opt.rank0:  # we are the reporting hub rank
            ...
            with open(appfile, "a") as f:
                f.write(", "+str(IB)+", "+str(OB)+", "+str(spcomm.opt._PHIter))
                f.write(", "+str((aph_end_time - ph_end_time).total_seconds()))

    # EF
    print("Running EF on a hub rank")
    if "hub_class" in opt_dict:  # we are hub rank
        if spcomm.opt.rank == spcomm.opt.rank0:  # we are the reporting hub rank
            creator_options = {"cb_data": cb_data}                
            ef = create_EF(all_scenario_names,
                           pysp2_callback,
                           creator_options)
            ...
            ee = dt.datetime.now()
            with open(appfile, "a") as f:
                f.write(", "+str(pyo.value(ef.EF_Obj)))
                f.write(", "+str((ee - aph_end_time).total_seconds()))
