...
class FWPH(mpisppy.phbase.PHBase):
    def __init__(
        ...
        assert (
            variable_probability is None
        ), "variable probability is not allowed with fwph"
    ...
    def fw_prep(self):
        ...
        if "point_creator" in self.FW_options:
            ...
            check = (
                True
                if "check_initial_points" not in self.FW_options
    ...
    def fwph_main(self):
        ...
        for itr in range(self.options["PHIterLimit"]):
            ...
            for name in self.local_subproblems:
                ...
                self._local_bound += (
                    self.local_subproblems[name]._mpisppy_probability * dual_bound
                )
            ...
            if self.spcomm:
                if self.spcomm.is_converged():
                    secs = time.time() - self.t0
                    self._output(itr + 1, self._local_bound, best_bound, np.nan, secs)
    ...
    def SDM(self, model_name):
        ...
        # a numerical problem).
        arb_scen_mip = self.local_scenarios[mip.scen_list[0]] if self.bundling else mip
        for node_name, ix in arb_scen_mip._mpisppy_data.nonant_indices:
            qp._mpisppy_model.W[node_name, ix]._value = arb_scen_mip._mpisppy_model.W[
                node_name, ix
            ].value
        ...
        # Algorithm 3 line 6
        xt = {
            ndn_i: (1 - alpha) * pyo.value(arb_scen_mip._mpisppy_model.xbars[ndn_i])
        ...
        for itr in range(self.FW_options["FW_iter_limit"]):
            ...
            for scenario_name in mip_source:
                ...
                for ndn_i, nonant in scen_mip._mpisppy_data.nonant_indices.items():
                    x_source = xt[ndn_i] if itr == 0 else nonant._value
                    scen_mip._mpisppy_model.W[ndn_i]._value = qp._mpisppy_model.W[
                        ndn_i
                    ]._value + scen_mip._mpisppy_model.rho[ndn_i]._value * (
                        x_source - scen_mip._mpisppy_model.xbars[ndn_i]._value
                    )
            ...
            if abs(val0) > 1e-9:
                stop_check = (val1 - val0) / abs(
                    val0
                )  # \Gamma^t in Boland, but normalized
            else:
                stop_check = val1 - val0  # \Gamma^t in Boland
            stop_check_tol = (
                self.FW_options["stop_check_tol"]
                if "stop_check_tol" in self.FW_options
                else 1e-4
            )
        ...
        for scenario_name in mip_source:
            ...
            for node_name, ix in scen_mip._mpisppy_data.nonant_indices:
                scen_mip._mpisppy_model.W[node_name, ix]._value = qp._mpisppy_model.W[
                    node_name, ix
                ]._value
    ...
    def _attach_indices(self):
        ...
        for name in self.local_subproblems.keys():
            ...
            if self.bundling:
                x_indices = [
                    (scenario_name, node_name, ix)
                    for scenario_name in mip.scen_list
                    for (node_name, ix) in self.local_scenarios[
                        scenario_name
                    ]._mpisppy_data.nonant_indices
                ]
                y_indices = [
                    (scenario_name, "LEAF", ix)
                    for scenario_name in mip.scen_list
    ...
    def _attach_MIP_QP_maps(self):
        ...
        for name in self.local_subproblems.keys():
            ...
            mip._mpisppy_data.mip_to_qp = {
                id(mip.nonant_vars[key]): qp.x[key]
                for key in mip._mpisppy_model.x_indices
            }
            mip._mpisppy_data.mip_to_qp.update(
                {
                    id(mip.leaf_vars[key]): qp.y[key]
                    for key in mip._mpisppy_model.y_indices
                }
            )
            qp._mpisppy_data.qp_to_mip = {
                id(qp.x[key]): mip.nonant_vars[key]
                for key in mip._mpisppy_model.x_indices
            }
            qp._mpisppy_data.qp_to_mip.update(
                {
                    id(qp.y[key]): mip.leaf_vars[key]
                    for key in mip._mpisppy_model.y_indices
                }
            )
        ...
        if self.bundling:
            for bundle_name, EF in self.local_subproblems.items():
                ...
                for scenario_name in EF.scen_list:
                    ...
                    nonant_dict = {
                        (scenario_name, ndn, ix): nonant
                        for (
                            ndn,
                            ix,
                        ), nonant in mip._mpisppy_data.nonant_indices.items()
                    }
                    EF.nonant_vars.update(nonant_dict)
                    # Leaf variables
                    leaf_var_dict = {
                        (scenario_name, "LEAF", ix): var
                        for ix, var in enumerate(self._get_leaf_vars(mip))
                    }
            ...
            for name, mip in self.local_scenarios.items():
                ...
                mip.leaf_vars = {
                    ("LEAF", ix): var for ix, var in enumerate(self._get_leaf_vars(mip))
                }
    ...
    def _check_initial_points(self):
        ...
        points = {key: value for block in init_pts for (key, value) in block.items()}
        ...
        def conv_rule(model, scenario_name, var_name):
            return model.x[var_name] + model.slack_plus[
                scenario_name, var_name
            ] - model.slack_minus[scenario_name, var_name] == pyo.quicksum(
                model.conv[scenario_name, ix] * points[scenario_name][ix][var_name]
                for ix in range(len(points[scenario_name]))
            )

        aux.comb = pyo.Constraint(conv_ix, rule=conv_rule)

        obj_expr = pyo.quicksum(aux.slack_plus.values()) + pyo.quicksum(
            aux.slack_minus.values()
        )
        ...
        check_tol = (
            self.FW_options["check_tol"]
            if "check_tol" in self.FW_options.keys()
            else 1e-4
        )
        if pyo.value(obj_expr) > check_tol:
            print("error.")
            raise ValueError(
                "The specified initial points do not satisfy the "
    ...
    def _compute_dual_bound(self):
        ...
        recv = np.array(0.0)
        self.comms["ROOT"].Allreduce([send, MPI.DOUBLE], [recv, MPI.DOUBLE], op=MPI.SUM)
    ...
    def _conv_diff(self):
        ...
        for name in self.local_subproblems.keys():
            mip = self.local_subproblems[name]
            arb_mip = self.local_scenarios[mip.scen_list[0]] if self.bundling else mip
            qp = self.local_QP_subproblems[name]
            diff_s = 0.0
            for node_name, ix in arb_mip._mpisppy_data.nonant_indices:
                qpx = qp.xr if self.bundling else qp.x
                diff_s += np.power(
                    pyo.value(qpx[node_name, ix])
                    - pyo.value(arb_mip._mpisppy_model.xbars[node_name, ix]),
                    2,
                )
        ...
        diff = np.array(diff)
        recv = np.array(0.0)
        self.comms["ROOT"].Allreduce([diff, MPI.DOUBLE], [recv, MPI.DOUBLE], op=MPI.SUM)
        return recv
    ...
    def _extract_objective(self, mip):
        ...
        if len(repn.nonlinear_vars) > 0:
            raise ValueError(
                "FWPH does not support models with nonlinear objective functions"
            )
        ...
        if repn.quadratic_vars:
            ...
            new += pyo.quicksum(
                (
                    coef * x * y
                    for coef, (x, y) in zip(repn.quadratic_coefs, quadratic_vars)
                )
            )
        return obj, new
        ...
        for name, scenario in self.local_scenarios.items():
            if self.bundling and strip_bundle_names:
                ...
                for ndn_ix, var in scenario._mpisppy_data.nonant_indices.items():
                    ...
                    scenario_weights[var_name] = scenario._mpisppy_model.W[ndn_ix].value
            else:
                scenario_weights = {
                    nonant.name: scenario._mpisppy_model.W[ndn_ix].value
        ...
        if self.cylinder_rank != 0:
            ...
            for node in scenario._mpisppy_node_list:
                for ix, var in enumerate(node.nonant_vardata_list):
                    ...
                    xbar_dict[var_name] = scenario._mpisppy_model.xbars[
                        node.name, ix
                    ].value
            return xbar_dict
        ...
        for name, mip in self.local_subproblems.items():
            ...
            for var in mip_vars:
                try:
                    ...
                    raise KeyError(
                        "Found variable named"
                        + var.name
                        + " in model "
                        + name
                        + " not contained in the "
                        "specified initial point dictionary"
                    ) from e
    ...
    def _initialize_QP_subproblems(self):
        ...
        for name, model in self.local_subproblems.items():
            if self.bundling:
                ...
                if has_init_pts:
                    raise RuntimeError(
                        "Cannot currently specify " "initial points while using bundles"
                    )
            else:
            ...
            if self.bundling:
                ...
                def nonant_rule(m, scenario_name, node_name, ix):
                    return m.x[scenario_name, node_name, ix] == m.xr[node_name, ix]
            ...
            if self.bundling:
                ...
                def x_rule(m, node_name, ix):
                    return (
                        -m.xr[node_name, ix]
                        + m.a[1] * model.ref_vars[node_name, ix].value
                        == 0
                    )

                def y_rule(m, scenario_name, node_name, ix):
                    return (
                        -m.y[scenario_name, node_name, ix]
                        + m.a[1] * model.leaf_vars[scenario_name, node_name, ix].value
                        == 0
                    )
                ...
                if has_init_pts:
                    ...
                    def x_rule(m, node_name, ix):
                        ...
                        return (
                            -m.x[node_name, ix]
                            + pyo.quicksum(
                                m.a[i + 1] * pts[i][nm] for i in range(len(pts))
                            )
                            == 0
                        )
                    ...
                    def x_rule(m, node_name, ix):
                        return (
                            -m.x[node_name, ix]
                            + m.a[1] * model.nonant_vars[node_name, ix].value
                            == 0
                        )
                    ...
                    def y_rule(m, node_name, ix):
                        return (
                            -m.y[node_name, ix]
                            + m.a[1] * model.leaf_vars["LEAF", ix].value
                            == 0
                        )
        ...
        for name in self.local_subproblems.keys():
            ...
            if self.bundling:
                ...
                for node_name, ix in naix:
                    ...
                    vals = [
                        mip.nonant_vars[scenario_name, node_name, ix].value
                        for scenario_name in mip.scen_list
                    ]
    ...
    def _is_timed_out(self):
        if self.cylinder_rank == 0:
            ...
            status = 1 if (time_elapsed > self.FW_options["time_limit"]) else 0
        else:
            status = None
    ...
    def _options_checks_fw(self):
        ...
        if len(losers) > 0:
            msg = "FW_options is missing the following key(s): " + ", ".join(losers)
        ...
        use_bundles = (
            "bundles_per_rank" in self.options and self.options["bundles_per_rank"] > 0
        )
        t_max = self.FW_options["FW_iter_limit"]
        specd_init_pts = (
            "point_creator" in self.FW_options.keys()
            and self.FW_options["point_creator"] is not None
        )

        if use_bundles and specd_init_pts:
            if t_max == 1:
                raise RuntimeError(
                    "Cannot use bundles and specify initial "
                    "points with t_max=1 at the same time."
                )
            else:
                if self.cylinder_rank == 0:
                    print(
                        "WARNING: Cannot specify initial points and use "
        ...
        if t_max == 1 and not specd_init_pts:
            raise RuntimeError(
                "FW_iter_limit set to 1. To ensure "
        ...
        if (
            "linearize_binary_proximal_terms" in self.options
            and self.options["linearize_binary_proximal_terms"]
        ):
            print(
                "Warning: linearize_binary_proximal_terms cannot be used "
        ...
        if (
            "linearize_proximal_terms" in self.options
            and self.options["linearize_proximal_terms"]
        ):
            print(
                "Warning: linearize_proximal_terms cannot be used "
                "with the FWPH algorithm. Ignoring..."
            )
    ...
    def _output(self, itr, bound, best_bound, diff, secs):
        if self.cylinder_rank == 0 and self.vb:
            print(
                "{itr:3d} {bound:12.4f} {best_bound:12.4f} {diff:12.4e} {secs:11.1f}s".format(
                    itr=itr, bound=bound, best_bound=best_bound, diff=diff, secs=secs
                )
            )
        if self.cylinder_rank == 0 and "save_file" in self.FW_options.keys():
            ...
            with open(fname, "a") as f:
                f.write(
                    "{itr:d},{bound:.16f},{best_bound:.16f},{diff:.16f},{secs:.16f}\n".format(
                        itr=itr,
                        bound=bound,
    ...
    def _output_header(self):
        if self.cylinder_rank == 0 and self.vb:
            print(
                "itr {bound:>12s} {bb:>12s} {cd:>12s} {tm:>12s}".format(
        ...
        if self.cylinder_rank == 0 and "save_file" in self.FW_options.keys():
            ...
            with open(fname, "a") as f:
                f.write(
                    "{itr:s},{bound:s},{bb:s},{diff:s},{secs:s}\n".format(
                        itr="Iteration",
                        bound="Bound",
                        bb="Best bound",
                        diff="Error",
    ...
    def save_weights(self, fname):
        ...
        """
        weights = self._gather_weight_dict(
            strip_bundle_names=self.bundling
        )  # None if rank != 0
        if self.cylinder_rank != 0:
            return
        ...
        for name, mip in self.local_subproblems.items():
            ...
            ph_term = pyo.quicksum(
                (
                    ...
                    + (m_source._mpisppy_model.rho[nni] / 2.0)
            ...
            for scenario_name in scens:
                ...
                for node in node_list:
                    node.nonant_vardata_list = [
                        ...
                        if self.bundling
                        else self.local_QP_subproblems[name].x[node.name, i]
