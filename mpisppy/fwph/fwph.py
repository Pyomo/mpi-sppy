...
class FWPH(mpisppy.phbase.PHBase):
    ...
    def fw_prep(self):
        ...
        if ('point_creator' in self.FW_options):
            ...
            self._use_rho_setter(verbose and self.cylinder_rank==self.cylinder_rank0)
        ...
        for itr in range(self.PHoptions['PHIterLimit']):
            ...
            if self.spcomm:
                if self.spcomm.is_converged():
                    ...
                    if (self.cylinder_rank == self.cylinder_rank0 and self.vb):
    ...
    def _output(self, itr, bound, best_bound, diff, secs):
        if (self.cylinder_rank == self.cylinder_rank0 and self.vb):
            print('{itr:3d} {bound:12.4f} {best_bound:12.4f} {diff:12.4e} {secs:11.1f}s'.format(
                    ...
                    diff=diff, secs=secs))
        if (self.cylinder_rank == self.cylinder_rank0 and 'save_file' in self.FW_options.keys()):
