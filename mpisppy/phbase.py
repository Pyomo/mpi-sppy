...
def _Compute_Xbar(opt, verbose=False):
    ...
    for k, s in opt.local_scenarios.items():
        logger.debug(
            "  top of assign xbar loop for {} on rank {}".format(k, opt.cylinder_rank)
        )
        nlens = s._mpisppy_data.nlens
        for node in s._mpisppy_node_list:
            ndn = node.name
            ...
            for i in range(nlen):
                ...
                if verbose:  # and opt.cylinder_rank == 0:
                    print(
                        "cylinder rank, scen, node, var, xbar:",
                        opt.cylinder_rank,
                        k,
                        ndn,
                        node.nonant_vardata_list[i].name,
...
def _Compute_Wbar(opt, verbose=False, repair=True):
    ...
    for k, s in opt.local_scenarios.items():
        ...
        for node in s._mpisppy_node_list:
            ...
            Wnonants_array = np.fromiter(
                (
                    pyo.value(s._mpisppy_model.W[idx])
                    for idx in s._mpisppy_data.nonant_indices
                    if idx[0] == ndn
                ),
                dtype="d",
                count=nlen,
    ...
    for k, s in opt.local_scenarios.items():
        logger.debug(
            "  top of Wbar loop for {} on rank {}".format(k, opt.cylinder_rank)
        )
        nlens = s._mpisppy_data.nlens
        for node in s._mpisppy_node_list:
            ndn = node.name
            ...
            for i in range(nlen):
                if abs(Wbars[i]) > opt.E1_tolerance and opt.cylinder_rank == 0:
                    print(
                        f"EW={Wbars[i]} (should be zero) for {node.nonant_vardata_list[i].name}"
                    )
                    if repair:
...
class PHBase(mpisppy.spopt.SPOpt):
    ...
    def Update_W(self, verbose):
        ...
        for k, s in self.local_scenarios.items():
            for ndn_i, nonant in s._mpisppy_data.nonant_indices.items():
                ...
                xdiff = nonant._value - s._mpisppy_model.xbars[ndn_i]._value
                s._mpisppy_model.W[ndn_i]._value += (
                    pyo.value(s._mpisppy_model.rho[ndn_i]) * xdiff
                )
                if verbose and self.cylinder_rank == 0:
                    print(
                        "rank, node, scen, var, W",
                        ndn_i[0],
                        k,
                        self.cylinder_rank,
                        nonant.name,
    ...
    def Update_z(self, verbose):
        ...
        for k, s in self.local_scenarios.items():
            for ndn_i, nonant in s._mpisppy_data.nonant_indices.items():
                xzdiff = nonant._value - s._mpisppy_model.z[ndn_i]._value
                s._mpisppy_model.z[ndn_i]._value += (
                    pyo.value(s._mpisppy_model.beta[ndn_i]) * xzdiff
                )
                if verbose and self.cylinder_rank == 0:
                    print(
                        "rank, node, scen, var, z",
                        ndn_i[0],
                        k,
                        self.cylinder_rank,
                        nonant.name,
    ...
    def _populate_W_cache(self, cache, padding):
        ...
        for model in self.local_scenarios.values():
            if (ci + len(model._mpisppy_data.nonant_indices)) >= len(cache):
                tlen = len(model._mpisppy_data.nonant_indices) * len(
                    self.local_scenarios
                )
                raise RuntimeError(
                    "W cache length mismatch detected by "
    ...
    def _use_rho_setter(self, verbose):
        ...
        skipped = 0
        rho_setter_kwargs = (
            self.options["rho_setter_kwargs"]
    ...
    def post_solve_bound(self, solver_options=None, verbose=False):
        ...
        if self.cylinder_rank == 0:
            print(
                "Warning: Lagrangian bounds might not be correct in certain "
    ...
    def _update_prox_approx(self):
        ...
        for sn, s in self.local_scenarios.items():
            persistent_solver = (
                s._solver_plugin if sputils.is_persistent(s._solver_plugin) else None
            )
    ...
    def attach_Ws_and_prox(self):
        ...
        for sname, scenario in self.local_scenarios.items():
            ...
            scenario._mpisppy_model.W = pyo.Param(
            ...
            scenario._mpisppy_model.W_on = pyo.Param(
                initialize=0, mutable=True, within=pyo.Binary
            )

            scenario._mpisppy_model.prox_on = pyo.Param(
                initialize=0, mutable=True, within=pyo.Binary
            )

            # note that rho is per var and scenario here
            scenario._mpisppy_model.rho = pyo.Param(
                scenario._mpisppy_data.nonant_indices.keys(),
                mutable=True,
    ...
    def attach_smoothing(self):
        ...
        for sname, scenario in self.local_scenarios.items():
            ...
            scenario._mpisppy_model.p = pyo.Param(
                scenario._mpisppy_data.nonant_indices.keys(),
                mutable=True,
                default=self.options["defaultPHp"],
            )

            scenario._mpisppy_model.beta = pyo.Param(
                scenario._mpisppy_data.nonant_indices.keys(),
                mutable=True,
    ...
    def W_disabled(self):
        assert hasattr(
            self.local_scenarios[self.local_scenario_names[0]]._mpisppy_model, "W_on"
        )
        return not bool(
            self.local_scenarios[self.local_scenario_names[0]]._mpisppy_model.W_on.value
        )

    @property
    def prox_disabled(self):
        assert hasattr(
            self.local_scenarios[self.local_scenario_names[0]]._mpisppy_model, "prox_on"
        )
        return not bool(
            self.local_scenarios[
                self.local_scenario_names[0]
            ]._mpisppy_model.prox_on.value
        )
        ...
        for sname, scenario in self.local_scenarios.items():
            ...
            if self._prox_approx:
                ...
                scenario._mpisppy_model.xsqvar = pyo.Var(
                    scenario._mpisppy_data.nonant_indices, dense=False, bounds=(0, None)
                )
            ...
            # Dual term (weights W)
            if add_duals:
                scenario._mpisppy_model.WExpr = pyo.Expression(
                    expr=sum(
                        scenario._mpisppy_model.W[ndn_i] * xvar
                ...
                for ndn_i, xvar in scenario._mpisppy_data.nonant_indices.items():
                    ...
                    if xvar.is_binary() and (lin_bin_prox or self._prox_approx):
                        ...
                        scenario._mpisppy_data.xsqvar_prox_approx[ndn_i] = (
                            ProxApproxManager(
                                ...
                                xvarsqrd,
                                xbars[ndn_i],
                                scenario._mpisppy_model.xsqvar_cuts,
                                ndn_i,
                            )
                        )
                    else:
                    ...
                    if add_smooth:
                        smooth_expr += (scenario._mpisppy_model.p[ndn_i] / 2.0) * (
                            xvarsqrd
                            - 2.0 * scenario._mpisppy_model.z[ndn_i] * xvar
                            + scenario._mpisppy_model.z[ndn_i] ** 2
                        )

                scenario._mpisppy_model.ProxExpr = pyo.Expression(expr=prox_expr)
                ph_term += (
                    scenario._mpisppy_model.prox_on * scenario._mpisppy_model.ProxExpr
                )

                if add_smooth:
                    # Adding smoothing term
                    scenario._mpisppy_model.SmoothExpr = pyo.Expression(
                        expr=smooth_expr
                    )
                    ph_term += (
                        scenario._mpisppy_model.prox_on
                        * scenario._mpisppy_model.SmoothExpr
                    )
    ...
    def PH_Prep(self, attach_duals=True, attach_prox=True, attach_smooth=0):
    ...
    def Iter0(self):
        ...
        if self.options["verbose"]:
            print(
                "About to call PH Iter0 solve loop on rank={}".format(
                    self.cylinder_rank
                )
            )
        ...
        if smooth_type == 2:
            for _, scenario in self.local_scenarios.items():
                for ndn_i, _ in scenario._mpisppy_data.nonant_indices.items():
                    scenario._mpisppy_model.p[ndn_i] *= scenario._mpisppy_model.rho[
                        ndn_i
                    ]
    ...
    def iterk_loop(self):
        ...
        for self._PHIter in range(1, max_iterations + 1):
            ...
            if dprogress:
                global_toc(
                    f"Initiating PH Iteration {self._PHIter}\n", self.cylinder_rank == 0
                )
    ...
    def post_loops(self, extensions=None):
        ...
        if self.ph_converger is not None and hasattr(
            self.ph_converger, "post_everything"
        ):
        ...
        if dtiming and self.cylinder_rank == 0:
            print("")
            print(
                "Cumulative execution time=%5.2f"
