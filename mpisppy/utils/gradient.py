...
class Find_Grad:
    ...
    def find_grad_cost(self):
        ...
        assert (
            self.cfg.xhatpath != ""
        ), "to compute gradient cost, you have to give an xhat path using --xhatpath"
        ...
        grad_dict = {
            sname: self.compute_grad(sname, scenario)
            for sname, scenario in self.ph_object.local_scenarios.items()
        }
        local_costs = {
            (sname, var.name): grad_dict[sname][node.name, ix]
            for (sname, scenario) in self.ph_object.local_scenarios.items()
            for node in scenario._mpisppy_node_list
        ...
        if self.ph_object.cylinder_rank == 0:
            self.c = {key: val for cost in costs for key, val in cost.items()}
        comm.Barrier()
...
def grad_cost_and_rho(mname, original_cfg):
    ...
    """
    if (original_cfg.grad_rho_file_out == "") and (
        original_cfg.grad_cost_file_out == ""
    ):
        raise RuntimeError(
            "Presently, grad-rho-file-out and grad-cost-file cannot both be empty"
        )
    ...
    scen_names_creator_args = inspect.getfullargspec(
