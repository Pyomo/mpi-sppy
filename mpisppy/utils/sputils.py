...
def scenario_tree_solution_writer(directory_name, scenario_name, scenario, bundling):
    with open(os.path.join(directory_name, scenario_name + ".csv"), "w") as f:
        for var in scenario.component_data_objects(
            ctype=(pyo.Var, pyo.Expression), descend_into=True, active=True, sort=True
        ):
            var_name = var.name
            if bundling:
                dot_index = var_name.find(".")
...
def write_spin_the_wheel_first_stage_solution(
    spcomm,
    opt_dict,
    solution_file_name,
...
def write_spin_the_wheel_tree_solution(
    spcomm,
    opt_dict,
    solution_directory_name,
...
def get_objs(scenario_instance, allow_none=False):
    ...
    if (len(scenario_objs) == 0) and not allow_none:
        raise RuntimeError(
            f"Scenario {scenario_instance.name} has no active " "objective functions."
        )
    if len(scenario_objs) > 1:
        print(
            "WARNING: Scenario",
            scenario_instance.name,
            "has multiple active " "objectives. Selecting the first objective.",
        )
...
def create_EF(
    ...
    uniform_specified = probs_specified and all(
        scen._mpisppy_probability == "uniform" for scen in scen_dict.values()
    )
    ...
    EF_instance = _create_EF_from_scen_dict(
        scen_dict, EF_name=EF_name, nonant_for_fixed_vars=nonant_for_fixed_vars
    )
    return EF_instance
...
def is_persistent(solver):
    return isinstance(
        solver, pyo.pyomo.solvers.plugins.solvers.persistent_solver.PersistentSolver
    )
...
def ef_ROOT_nonants_npy_serializer(ef, filename):
    ...
    """
    root_nonants = np.fromiter(
        (v for ndn, var, v in ef_nonants(ef) if ndn == "ROOT"), float
    )
    np.save(filename, root_nonants)


def write_ef_first_stage_solution(
    ef, solution_file_name, first_stage_solution_writer=first_stage_nonant_writer
):
    """
    ...
    if global_rank == 0:
        ...
        first_stage_solution_writer(
            solution_file_name, representative_scenario, bundling=False
        )


def write_ef_tree_solution(
    ef,
    solution_directory_name,
    scenario_tree_solution_writer=scenario_tree_solution_writer,
):
    ...
    if global_rank == 0:
        ...
        for scenario_name, scenario in ef_scenarios(ef):
            scenario_tree_solution_writer(
                solution_directory_name, scenario_name, scenario, bundling=False
            )
...
def option_string_to_dict(ostr):
    ...
    for this_option_string in ostr.split():
        ...
        if len(this_option_pieces) == 2:
            ...
            raise RuntimeError(
                "Illegally formed subsolve directive"
...
def scens_to_ranks(scen_count, n_proc, rank, branching_factors=None):
    ...
    if branching_factors is None:
        ...
        (
            scenario_names_to_ranks,
            slices,
        ) = tree.scen_name_to_rank(n_proc, rank)
    ...
    def __init__(self, Parent, scenfirst, scenlast, desc_leaf_dict, name):
        ...
        if len(desc_leaf_dict) == 1 and list(desc_leaf_dict.keys()) == ["ROOT"]:
            ...
            if len(desc_leaf_dict) < numscens:
                raise RuntimeError(
                    f"There are more scenarios ({numscens}) than remaining leaves, for the node {name}"
                )
            # make children
            first = scenfirst
            self.kids = list()
            ...
            for i in range(len(desc_leaf_dict)):
                ...
                self.kids.append(
                    _TreeNode(self, first, last, child_leaf_dict, childname)
                )
                first += child_scens_num
            if last != scenlast:
                print("numscens, last, scenlast", numscens, last, scenlast)
                raise RuntimeError(
                    f"Tree node did not initialize correctly for node {name}"
                )
...
class _ScenTree:
    def __init__(self, all_nodenames, ScenNames):
        ...
        self.NumStages = 2 if all_nodenames == ["ROOT"] else self.rootnode.stage_max()
    ...
    def scen_names_to_ranks(self, n_proc):
        ...
        # scenario index -> rank
        list_of_ranks_per_scenario_idx = [
            rank for rank, scen_idxs in enumerate(slices) for _ in scen_idxs
        ]

        scenario_names_to_rank["ROOT"] = {
            s: rank for s, rank in zip(self.ScenNames, list_of_ranks_per_scenario_idx)
        }
        ...
        def _recurse_do_node(node):
            for child in node.kids:
                ...
                ranks_in_node = list_of_ranks_per_scenario_idx[
                    first_scen_idx : last_scen_idx + 1
                ]
                ...
                # node's comm
                within_comm_ranks_in_node = [
                    (rank - minimum_rank_in_node) for rank in ranks_in_node
                ]

                scenarios_in_nodes = self.ScenNames[first_scen_idx : last_scen_idx + 1]

                scenario_names_to_rank[child.name] = {
                    s: rank
                    for s, rank in zip(scenarios_in_nodes, within_comm_ranks_in_node)
                }
...
def attach_root_node(
    model, firstobj, varlist, nonant_ef_suppl_list=None, do_uniform=True
):
...
def check4losses(
    ...
    # not stage presence...
    stagepresents = {
        stage: [False for _ in range(numscens)]
        for stage in range(len(branching_factors))
    }
