...
class LShapedCutGeneratorData(bc.BendersCutGeneratorData):
    ...
    def set_ls(self, ls):
        ...
        self._subproblem_ndx_map = dict.fromkeys(
            range(len(self.ls.local_scenario_names))
        )
        for s in self._subproblem_ndx_map.keys():
            self._subproblem_ndx_map[s] = self.ls.all_scenario_names.index(
                self.ls.local_scenario_names[s]
            )
    ...
    def add_subproblem(
        ...
        if subproblem_fn_kwargs["scenario_name"] in self.ls.local_scenario_names:
            ...
            if isinstance(subproblem_solver, PersistentSolver):
                set_instance_retry(
                    subproblem, subproblem_solver, subproblem_fn_kwargs["scenario_name"]
                )
