...
# Copied with modification from pysp/scenariotree/tree_structure_model.py
...
def CreateAbstractScenarioTreeModel():
    ...
    model.Children = Set(model.Nodes, within=model.Nodes, initialize=[], ordered=True)
    model.ConditionalProbability = Param(model.Nodes, within=UnitInterval, mutable=True)

    model.Scenarios = Set(ordered=True)
    model.ScenarioLeafNode = Param(model.Scenarios, within=model.Nodes, mutable=True)

    model.StageVariables = Set(model.Stages, initialize=[], ordered=True)

    model.NodeVariables = Set(model.Nodes, initialize=[], ordered=True)

    model.StageCost = Param(model.Stages, within=Any, mutable=True, default=None)
    model.NodeCost = Param(model.Nodes, within=Any, mutable=True, default=None)

    # DEPRECATED
    model.StageCostVariable = Param(model.Stages, within=Any, mutable=True)
    ...
    model.StageDerivedVariables = Set(model.Stages, initialize=[], ordered=True)
