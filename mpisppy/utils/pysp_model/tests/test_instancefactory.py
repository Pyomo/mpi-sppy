...
# Copied with modification from pysp/tests/unit/test_instancefactory.py
...
from mpisppy.utils.pysp_model.instance_factory import ScenarioTreeInstanceFactory
...
def setUpModule():
    ...
    reference_test_model = import_file(
        join(testdatadir, "reference_test_model.py")
    ).model
...
class Test(unittest.TestCase):
    ...
    def _check_factory(self, factory):
        ...
        instances = factory.construct_instances_for_scenario_tree(
            scenario_tree, compile_scenario_instances=True, verbose=True
        )
        self.assertEqual(len(instances), 3)
        self.assertEqual(instances["s1"].p(), 1)
        self.assertEqual(instances["s2"].p(), 2)
        self.assertEqual(instances["s3"].p(), 3)
        self.assertEqual(
            factory.construct_scenario_instance("s1", scenario_tree, verbose=True).p(),
            1,
        )
        ...
        with self.assertRaises(ValueError):
            scenario_tree = factory.generate_scenario_tree(
                random_bundles=1000, verbose=True
            )
        with self.assertRaises(ValueError):
            scenario_tree = factory.generate_scenario_tree(
                random_bundles=2, bundles={}, verbose=True
            )
        ...
        scenario_tree = factory.generate_scenario_tree(
            downsample_fraction=0.1, verbose=True
        )
        scenario_tree = factory.generate_scenario_tree(
            bundles={"b1": ["s1"], "b2": ["s2"], "b3": ["s3"]}, verbose=True
        )
        self.assertEqual(scenario_tree.contains_bundles(), True)
        ...
        if factory._scenario_tree is not None:
            ...
            with self.assertRaises(ValueError):
                scenario_tree = factory.generate_scenario_tree(
                    bundles=join(testdatadir, "bundles.dat"), verbose=True
                )
        else:
            scenario_tree = factory.generate_scenario_tree(
                bundles=join(testdatadir, "bundles.dat"), verbose=True
            ...
            self.assertEqual(len(scenario_tree.bundles), 3)
            scenario_tree = factory.generate_scenario_tree(
                bundles=join(testdatadir, "bundles"), verbose=True
            )
            self.assertEqual(scenario_tree.contains_bundles(), True)
            self.assertEqual(len(scenario_tree.bundles), 3)
            if (factory.data_directory() is not None) and exists(
                join(factory.data_directory(), "bundles.dat")
            ):
                scenario_tree = factory.generate_scenario_tree(
                    bundles="bundles.dat", verbose=True
                )
                self.assertEqual(scenario_tree.contains_bundles(), True)
                self.assertEqual(len(scenario_tree.bundles), 3)
                scenario_tree = factory.generate_scenario_tree(
                    bundles="bundles", verbose=True
                )
                self.assertEqual(scenario_tree.contains_bundles(), True)
                self.assertEqual(len(scenario_tree.bundles), 3)
            with self.assertRaises(ValueError):
                scenario_tree = factory.generate_scenario_tree(
                    bundles="bundles.notexists", verbose=True
                )
    ...
    def test_init1(self):
        ...
        with ScenarioTreeInstanceFactory(
            ...
            scenario_tree=join(testdatadir, "reference_test_scenario_tree.dat"),
        ) as factory:
    ...
    def test_init1_default(self):
        ...
        with ScenarioTreeInstanceFactory(
            model=testdatadir, scenario_tree=testdatadir
        ) as factory:
            self.assertEqual(len(factory._archives), 0)
            self.assertTrue(factory.model_directory() is not None)
            self.assertTrue(factory.scenario_tree_directory() is not None)
    ...
    def test_init2(self):
        self.assertTrue("reference_test_model" not in sys.modules)
        with ScenarioTreeInstanceFactory(
            model=join(testdatadir, "archive_test.tgz,reference_test_model.py"),
            scenario_tree=join(testdatadir, "reference_test_scenario_tree.dat"),
        ) as factory:
    ...
    def test_init2_default(self):
        self.assertTrue("ReferenceModel" not in sys.modules)
        with ScenarioTreeInstanceFactory(
            model=join(testdatadir, "archive_test.tgz,"), scenario_tree=testdatadir
        ) as factory:
            self.assertEqual(len(factory._archives), 1)
            self.assertTrue(factory.model_directory() is not None)
            self.assertTrue(factory.scenario_tree_directory() is not None)
    ...
    def test_init3(self):
        ...
        with ScenarioTreeInstanceFactory(
            model=join(testdatadir, "reference_test_model.py"),
            scenario_tree=join(
                testdatadir, "archive_test.tgz,reference_test_scenario_tree.dat"
            ),
        ) as factory:
            self.assertEqual(len(factory._archives), 1)
            self.assertTrue(factory.model_directory() is not None)
    ...
    def test_init3_default(self):
        self.assertTrue("ReferenceModel" not in sys.modules)
        with ScenarioTreeInstanceFactory(
            model=testdatadir, scenario_tree=join(testdatadir, "archive_test.tgz")
        ) as factory:
            self.assertEqual(len(factory._archives), 1)
            self.assertTrue(factory.model_directory() is not None)
            self.assertTrue(factory.scenario_tree_directory() is not None)
    ...
    def test_init4(self):
        ...
        with ScenarioTreeInstanceFactory(
            model=join(testdatadir, "archive_test.tgz,reference_test_model.py"),
            scenario_tree=join(
                testdatadir, archive_copy + ",reference_test_scenario_tree.dat"
            ),
        ) as factory:
    ...
    def test_init4_default(self):
        ...
        with ScenarioTreeInstanceFactory(
            ...
            scenario_tree=join(testdatadir, archive_copy + ","),
        ) as factory:
            self.assertEqual(len(factory._archives), 2)
            self.assertTrue(factory.model_directory() is not None)
            self.assertTrue(factory.scenario_tree_directory() is not None)
    ...
    def test_init5_default(self):
        ...
        with ScenarioTreeInstanceFactory(
            model=join(testdatadir, "archive_test.tgz,"),
            scenario_tree=join(testdatadir, "archive_test.tgz,"),
        ) as factory:
            self.assertEqual(len(factory._archives), 1)
            self.assertTrue(factory.model_directory() is not None)
            self.assertTrue(factory.scenario_tree_directory() is not None)
    ...
    def test_init6(self):
        self.assertTrue("reference_test_model" not in sys.modules)
        scenario_tree_model = CreateAbstractScenarioTreeModel().create_instance(
        ...
        with self.assertRaises(TypeError):
            with ScenarioTreeInstanceFactory(
                model=join(testdatadir, "reference_test_model.py"), scenario_tree=int
            ):
                pass
        with self.assertRaises(ValueError):
            with ScenarioTreeInstanceFactory(
                model=join(testdatadir, "reference_test_model.py"), scenario_tree=None
            ):
                pass
        with self.assertRaises(TypeError):
            with ScenarioTreeInstanceFactory(
                model=None, scenario_tree=scenario_tree_model
            ):
                pass
    ...
    def test_init7(self):
        ...
        with self.assertRaises(ValueError):
            with ScenarioTreeInstanceFactory(
                model=reference_test_model, scenario_tree=scenario_tree_model
            ) as factory:
                pass
        with ScenarioTreeInstanceFactory(
    ...
    def test_init8(self):
        ...
        with ScenarioTreeInstanceFactory(
            model=reference_test_model,
            scenario_tree=join(testdatadir, "reference_test_scenario_tree_model.py"),
            data=testdatadir,
        ) as factory:
            self.assertTrue(factory.model_directory() is None)
            self.assertTrue(factory.scenario_tree_directory() is not None)
    ...
    def test_init9(self):
        ...
        with ScenarioTreeInstanceFactory(
            model=join(testdatadir, "reference_test_model_with_callback.py"),
            scenario_tree=join(testdatadir, "reference_test_scenario_tree.dat"),
        ) as factory:
            self.assertEqual(len(factory._archives), 0)
            self.assertTrue(factory.model_directory() is not None)
    ...
    def test_init10(self):
        with ScenarioTreeInstanceFactory(
            model=testdatadir,
            scenario_tree=join(testdatadir, "reference_test_scenario_tree.dat"),
            data=join(testdatadir, "yaml_data"),
        ) as factory:
            self.assertEqual(len(factory._archives), 0)
            self.assertTrue(factory.model_directory() is not None)
            self.assertTrue(factory.scenario_tree_directory() is not None)
            self.assertTrue(factory.data_directory(), join(testdatadir, "yaml_data"))
            self._check_factory(factory)
        self.assertEqual(len(factory._archives), 0)
    ...
    def test_init12(self):
        ...
        with ScenarioTreeInstanceFactory(
            model=scenario_model_callback, scenario_tree=scenario_tree_model
        ) as factory:
            self.assertTrue(factory.model_directory() is None)
            self.assertTrue(factory.scenario_tree_directory() is None)
            self._check_factory(factory)
    ...
    def test_init13(self):
        ...
        with ScenarioTreeInstanceFactory(
            model=model, scenario_tree=scenario_tree_model
        ) as factory:
            self.assertTrue(factory.model_directory() is None)
            self.assertTrue(factory.scenario_tree_directory() is None)
    ...
    def test_init14(self):
        ...
        with ScenarioTreeInstanceFactory(
            model=reference_test_model, scenario_tree=scenario_tree, data=testdatadir
        ) as factory:
            self._check_factory(factory)
        self.assertEqual(factory._closed, True)
        self.assertEqual(len(factory._archives), 0)
    ...
    def test_init15(self):
        ...
        with ScenarioTreeInstanceFactory(
            model=join(testdatadir, "reference_test_model.py"),
            scenario_tree=join(testdatadir, "ScenarioStructure.py"),
        ) as factory:
            self.assertEqual(len(factory._archives), 0)
            self.assertTrue(factory.model_directory() is not None)
            self.assertTrue(factory.scenario_tree_directory() is not None)
    ...
    def test_init16(self):
        ...
        nx_tree = import_file(os.path.join(testdatadir, "ScenarioStructure.py")).G
        with ScenarioTreeInstanceFactory(
            model=join(testdatadir, "reference_test_model.py"), scenario_tree=nx_tree
        ) as factory:
            self.assertEqual(len(factory._archives), 0)
            self.assertTrue(factory.model_directory() is not None)
            self.assertTrue(factory.scenario_tree_directory() is None)
    ...
    def test_init18(self):
        ...
        with ScenarioTreeInstanceFactory(
            model=scenario_model_callback, scenario_tree=nx_tree
        ) as factory:
            self.assertTrue(factory.model_directory() is None)
            self.assertTrue(factory.scenario_tree_directory() is None)
            self._check_factory(factory)
    ...
    def test_init20(self):
        ...
        with ScenarioTreeInstanceFactory(
            model=scenario_model_callback, scenario_tree=my_scenario_tree
        ) as factory:
            self.assertTrue(factory.model_directory() is None)
            self.assertTrue(factory.scenario_tree_directory() is None)
            self._check_factory(factory)
    ...
    def test_init21(self):
        ...
        with ScenarioTreeInstanceFactory(
            model=join(testdatadir, "both_callbacks.py"),
            scenario_tree=join(testdatadir, "both_callbacks.py"),
        ) as factory:
            self.assertEqual(len(factory._archives), 0)
            self.assertTrue(factory.model_directory() is not None)
            self.assertTrue(factory.scenario_tree_directory() is not None)
    ...
    def test_init22(self):
        self.assertTrue("both_callbacks" not in sys.modules)
        with ScenarioTreeInstanceFactory(
            model=join(testdatadir, "both_callbacks.py"), scenario_tree=None
        ) as factory:
            self.assertEqual(len(factory._archives), 0)
            self.assertTrue(factory.model_directory() is not None)
            self.assertTrue(factory.scenario_tree_directory() is not None)
