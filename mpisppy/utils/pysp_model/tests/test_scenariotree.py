...
from mpisppy.utils.pysp_model.pysp_model import (
    _get_nonant_list,
    _get_derived_nonant_list,
)
from pyomo.core import ConcreteModel, Set, Var, Expression, Objective, Block, value
...
class TestScenarioTreeFromNetworkX(unittest.TestCase):
    ...
    def test_two_stage(self):
        ...
        self.assertEqual(sorted(list(model.Stages)), sorted(["Stage1", "Stage2"]))
        self.assertEqual(
            sorted(list(model.Nodes)), sorted(["Root", "Child1", "Child2"])
        )
        self.assertEqual(
            sorted(list(model.Children["Root"])), sorted(["Child1", "Child2"])
        )
        self.assertEqual(sorted(list(model.Children["Child1"])), sorted([]))
        self.assertEqual(sorted(list(model.Children["Child2"])), sorted([]))
        self.assertEqual(sorted(list(model.Scenarios)), sorted(["Child1", "Child2"]))
    ...
    def test_two_stage_more_node_attributes(self):
        ...
        G.add_node("Child1", cost="c2", variables=["q"], derived_variables=["z"])
        G.add_edge("Root", "Child1", weight=0.8)
        G.add_node("Child2", cost="c2", variables=["q"], derived_variables=["z"])
        G.add_edge("Root", "Child2", weight=0.2)
        model = ScenarioTreeModelFromNetworkX(G)
        self.assertEqual(sorted(list(model.Stages)), sorted(["Stage1", "Stage2"]))
        self.assertEqual(
    ...
    def test_multi_stage(self):
        ...
        model = ScenarioTreeModelFromNetworkX(G, edge_probability_attribute=None)
        self.assertEqual(
            sorted(list(model.Stages)), sorted(["Stage1", "Stage2", "Stage3"])
        )
        self.assertEqual(
        ...
        self.assertEqual(
            sorted(list(model.Scenarios)), sorted([4, 5, 6, 7, 8, 9, 10, 11, 12])
        )
    ...
    def test_unbalanced(self):
        ...
        self.assertEqual(sorted(list(model.Nodes)), sorted(["R", "0", "1", "00", "01"]))
        self.assertEqual(sorted(list(model.Children["R"])), sorted(["0", "1"]))
        self.assertEqual(sorted(list(model.Children["0"])), sorted(["00", "01"]))
        ...
        self.assertEqual(sorted(list(model.Scenarios)), sorted(["00", "01", "1"]))
    ...
    def test_bundles(self):
        ...
        self.assertEqual(sorted(list(model.Stages)), sorted(["Stage1", "Stage2"]))
        self.assertEqual(
            sorted(list(model.Nodes)), sorted(["r", "u0", "u1", "u2", "u3"])
        )
        self.assertEqual(
            sorted(list(model.Children["r"])), sorted(["u0", "u1", "u2", "u3"])
        )
        for i in range(4):
            self.assertEqual(sorted(list(model.Children["u" + str(i)])), sorted([]))
        self.assertEqual(
            sorted(list(model.Scenarios)), sorted(["u0", "u1", "u2", "u3"])
        )
        self.assertEqual(value(model.ConditionalProbability["r"]), 1.0)
        for i in range(4):
            self.assertEqual(value(model.ConditionalProbability["u" + str(i)]), 0.25)
        ...
        for k, bundle_name in enumerate(model.Bundles):
            self.assertEqual(
                list(model.BundleScenarios[bundle_name]),
    ...
    def test_bundles_incomplete(self):
        ...
        self.assertEqual(
            list(model.BundleScenarios["B"]), ["u" + str(i) for i in range(4)]
        )
