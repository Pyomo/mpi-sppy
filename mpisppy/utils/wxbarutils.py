...
def write_W_to_file(PHB, fname, sep_files=False):
    ...
    if sep_files:
        for sname, scenario in PHB.local_scenarios.items():
            scenario_Ws = {
                var.name: pyo.value(scenario._mpisppy_model.W[node.name, ix])
        ...
        local_Ws = {
            (sname, var.name): pyo.value(scenario._mpisppy_model.W[node.name, ix])
            for (sname, scenario) in PHB.local_scenarios.items()
            for node in scenario._mpisppy_node_list
            for (ix, var) in enumerate(node.nonant_vardata_list)
        }
...
def set_W_from_file(fname, PHB, rank, sep_files=False, disable_check=False):
    ...
    if sep_files:
        ...
        w_val_dict = _parse_W_csv(
            fname, scenario_names_local, scenario_names_global, rank
        )
...
def _parse_W_csv(fname, scenario_names_local, scenario_names_global, rank):
    ...
    if missing:
        raise RuntimeError(
            "rank " + str(rank) + " could not find the following "
...
def _check_W(w_val_dict, PHB, rank):
    ...
    probs = {
        name: model._mpisppy_probability
        for (name, model) in PHB.local_scenarios.items()
    }

    checks = dict()
    for vname in vn_model:  # Ensured vn_model = vn_provided
        checks[vname] = sum(
            probs[name] * w_val_dict[name][vname] for name in PHB.local_scenarios.keys()
        )
    ...
    if rank == 0:
        for vname in vn_model:
            ...
            if abs(dual) > 1e-7:
                raise RuntimeError(
                    "Provided weights do not satisfy "
...
def write_xbar_to_file(PHB, fname):
    ...
    scenario = PHB.local_scenarios[sname]
    xbars = {
        var.name: pyo.value(scenario._mpisppy_model.xbars[node.name, ix])
        for node in scenario._mpisppy_node_list
        for (ix, var) in enumerate(node.nonant_vardata_list)
    }
    with open(fname, "a") as f:
...
def _check_xbar(xbar_val_dict, PHB):
    ...
    if set1:
        raise RuntimeError(
            "Could not find the following required variable "
            "values in the provided input file: " + ", ".join([v for v in set1])
        )
    set2 = provided_vars.difference(var_names)
    if set2:
        print(
            "Ignoring the following variables values provided in the "
...
def ROOT_xbar_npy_serializer(PHB, fname):
    ...
    root_xbar_list = [
        pyo.value(arbitrary_scen._mpisppy_model.xbars["ROOT", ix])
