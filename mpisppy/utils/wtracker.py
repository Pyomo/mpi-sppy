...
class WTracker:
    ...
    def __init__(self, PHB):
        ...
        self.varnames = [
            var.name
            for node in arbitrary_scen._mpisppy_node_list
    ...
    def grab_local_Ws(self):
        ...
        if hasattr(self.PHB, "_PHIter"):  # update the iter value
            ...
            self.ph_iter = self.PHB.A_iter
        scenario_Ws = {
            sname: [w._value for w in scenario._mpisppy_model.W.values()]
    ...
    def compute_moving_stats(self, wlen, offsetback=0):
        ...
        if li - fi < wlen:
            return (
                f"WARNING: Not enough iterations ({cI}) for window len {wlen} and"
            ...
            for idx, varname in enumerate(self.varnames):
                for sname in self.PHB.local_scenario_names:
                    wlist[(varname, sname)] = [
                        self.local_Ws[i][sname][idx] for i in range(fi, li + 1)
                    ]
                    window_stats[(varname, sname)] = (
                        np.mean(wlist[(varname, sname)]),
                        np.std(wlist[(varname, sname)]),
                    )
            return wlist, window_stats

    def report_by_moving_stats(
        self, wlen, reportlen=None, stdevthresh=None, file_prefix=""
    ):
        ...
        """
        fname = (
            f"{file_prefix}_summary_iter{self.ph_iter}_rank{self.PHB.global_rank}.txt"
        )
        stname = (
            f"{file_prefix}_stdev_iter{self.ph_iter}_rank{self.PHB.global_rank}.csv"
        )
        cvname = f"{file_prefix}_cv_iter{self.ph_iter}_rank{self.PHB.global_rank}.csv"

        if self.PHB.cylinder_rank == 0:
            print(
                f"Writing (a) W tracker report(s) to files with names like {fname}, {stname}, and {cvname}"
            )
        with open(fname, "w") as fil:
            fil.write(f"Moving Stats W Report at iteration {self.ph_iter}\n")
            fil.write(
                f"    {len(self.varnames)} nonants\n"
        ...
        if not isinstance(wstats, str):
            Wsdf = pd.DataFrame.from_dict(
                wstats, orient="index", columns=["mean", "stdev"]
            )
            ...
            with open(fname, "a") as fil:
                ...
                fil.write(
                    f"Sorted by windowed stdev, row limit={reportlen}, window len={wlen} in {stname}\n"
                )
            by_stdev = Wsdf.sort_values(by="stdev", ascending=False)
            by_stdev[0:reportlen].to_csv(
                path_or_buf=stname, header=True, index=True, index_label=None, mode="w"
            )

            # scaled
            Wsdf["absCV"] = np.where(
                Wsdf["mean"] != 0, Wsdf["stdev"] / abs(Wsdf["mean"]), np.nan
            )
            ...
            with open(fname, "a") as fil:
                ...
                fil.write(
                    f"  Sorted by windowed abs CV, row limit={reportlen}, window len={wlen} in {cvname}\n"
                )
            by_absCV = Wsdf.sort_values(by="absCV", ascending=False)
            by_absCV[0:reportlen].to_csv(
                path_or_buf=cvname, header=True, index=True, index_label=None, mode="w"
            )
    ...
    def check_cross_zero(self, wlen, offsetback=0):
        ...
        if li - fi < wlen:
            return (
                f"WTRACKER WARNING: Not enough iterations ({cI}) for window len {wlen} and"
                f" offsetback {offsetback}\n"
            )
        else:
    ...
    def check_w_stdev(self, wlen, stdevthresh, offsetback=0):
        ...
        if li - fi < wlen:
            ...
            for idx, varname in enumerate(self.varnames):
                for sname in self.PHB.local_scenario_names:
                    if np.abs(window_stats[(varname, sname)][1]) >= np.abs(
                        stdevthresh * window_stats[(varname, sname)][0]
                    ):  # stdev scaled by mean
