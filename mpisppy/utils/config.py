...
class Config(pyofig.ConfigDict):
    ...
    def add_to_config(
        ...
        if name in self:
            ...
            c = self.declare(
                name,
                pyofig.ConfigValue(
                    description=description, domain=domain, default=default
                ),
            )
            if argparse:
    ...
    def add_and_assign(self, name, description, domain, default, value, complain=False):
        ...
        if name in self:
            if complain:
                print(
                    f"Duplicate {name} will not be added to self by add_and_assign {value}."
                )
                # raise RuntimeError(f"Trying to add duplicate {name} to self.")
        else:
    ...
    def _common_args(self):
        raise RuntimeError(
            "_common_args is no longer used. See comments at top of config.py"
        )
    ...
    def popular_args(self):
        ...
        self.add_to_config(
            "verbose", description="verbose output", domain=bool, default=False
        )
    ...
    def make_parser(self, progname=None, num_scens_reqd=False):
        raise RuntimeError(
            "make_parser is no longer used. See comments at top of config.py"
        )
    ...
    def _basic_multistage(self, progname=None, num_scens_reqd=False):
        raise RuntimeError(
            "_basic_multistage is no longer used. See comments at top of config.py"
        )
    ...
    def make_multistage_parser(self, progname=None):
        raise RuntimeError(
            "make_multistage_parser is no longer used. See comments at top of config.py"
        )
    ...
    def fwph_args(self):
        self.add_to_config(
            "fwph", description="have an fwph spoke", domain=bool, default=False
        )
    ...
    def ph_ob_args(self):
        ...
        self.add_to_config(
            "ph_ob_initial_rho_rescale_factor",
    ...
    def mult_rho_args(self):
        ...
        self.add_to_config(
            "mult_rho_convergence_tolerance",
        ...
        self.add_to_config(
            "mult_rho_update_stop_iteration",
        ...
        self.add_to_config(
            "mult_rho_update_start_iteration",
    ...
    def xhatxbar_args(self):
        self.add_to_config(
            "xhatxbar", description="have an xhatxbar spoke", domain=bool, default=False
        )
    ...
    def slammax_args(self):
        ...
        self.add_to_config(
            "slammax", description="have a slammax spoke", domain=bool, default=False
        )
    ...
    def slammin_args(self):
        ...
        self.add_to_config(
            "slammin", description="have a slammin spoke", domain=bool, default=False
        )
    ...
    def cross_scenario_cuts_args(self):
        ...
        self.add_to_config(
            "cross_scenario_iter_cnt",
            description="cross scen check bound improve iterations " "(default 4)",
            domain=int,
            default=4,
        )
    ...
    def gradient_args(self):
        self.add_to_config(
            "xhatpath", description="path to npy file with xhat", domain=str, default=""
        )
        self.add_to_config(
            "grad_cost_file_out",
    ...
    def tracking_args(self):
        ...
        self.add_to_config(
            "track_convergence",
            description="Adds convergence tracking ie" " gaps and bounds (default 0)",
            domain=int,
            default=0,
        )
        self.add_to_config(
