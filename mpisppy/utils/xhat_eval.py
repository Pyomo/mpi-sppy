...
mpisppy.log.setup_logger(
    "mpisppy.utils.xhat_eval", "xhateval.log", level=logging.CRITICAL
)
...
class Xhat_Eval(mpisppy.spopt.SPOpt):
    ...
    def solve_loop(
        ...
        for k, s in s_source.items():
            ...
            pyomo_solve_time = self.solve_one(
                solver_options,
                k,
                s,
                dtiming=dtiming,
    ...
    def Eobjective(self, verbose=False, fct=None):
        ...
        if not hasattr(self, "objs_dict"):
            raise RuntimeError(
                "Values of the objective functions for each scenario"
    ...
    def evaluate_one(self, nonant_cache, scenario_name, s):
        ...
        solver_options = (
            self.options["solver_options"] if "solver_options" in self.options else None
        )
        k = scenario_name
        self.solve_one(
            solver_options,
    ...
    def calculate_incumbent(self, fix_nonants=True, verbose=False):
        ...
        self.solve_loop(solver_options=self.current_solver_options, verbose=verbose)
...
if __name__ == "__main__":
    print(
        "For an example of the use of Xhat_Eval see, e.g., zhat4xhat.py, "
