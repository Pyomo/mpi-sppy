...
class Synchronizer(object):
    ...
    def __init__(
        self, comms, Lens, work_fct, rank, sleep_secs, asynch=False, listener_gigs=None
    ):
        self.asynch = asynch
        self.comms = comms
        self.Lens = Lens
    ...
    def run(self, args, kwargs):
        if self.asynch:
            ...
            listener = threading.Thread(
                name="listener", target=Synchronizer.listener_daemon, args=listenargs
            )
    ...
    def _check_Lens(self, local_data_in, global_data_out, redname, cname):
        ...
        if len(local_data_in[redname][cname]) != self.Lens[redname][cname]:
            self.global_quitting = 1
            print(
                "\nERROR listener_util:cname={} len in={} Lens={}".format(
                    cname, len(local_data_in[redname][cname]), self.Lens[redname][cname]
                )
            )
        if len(global_data_out[redname][cname]) != self.Lens[redname][cname]:
            self.global_quitting = 1
            print(
                "\nERROR listener_util:cname={} len out={} Lens={}".format(
                    cname,
                    len(global_data_out[redname][cname]),
        ...
        if self.asynch:
            ...
            for redname in reds:
                for cname in self.Lens[redname].keys():
                    if self.Lens[redname][cname] == 0:
                        continue
                    self._check_Lens(local_data_in, global_data_out, redname, cname)
                    if keep_up:
                    ...
                    np.copyto(
                        self.local_data[redname][cname], local_data_in[redname][cname]
                    )
            ...
            for redname in self.Lens.keys():
                for cname in self.Lens[redname].keys():
                    ...
                    comm.Allreduce(
                        [local_data_in[cname], mpi.DOUBLE],
        ...
        if self.asynch:
            ...
            for cname in self.Lens[redname].keys():
                if self.Lens[redname][cname] == 0:
                    continue
                np.copyto(
                    global_data_out[redname][cname], self.global_data[redname][cname]
                )  # dst, src
        ...
        while synchronizer.global_quitting == 0:
            ...
            for redname in synchronizer.Lens.keys():
                for cname in synchronizer.Lens[redname].keys():
                    ...
                    logging.debug(
                        "  redname %s cname %s pre-reduce on rank %d"
                        % (redname, cname, rank)
                    )
                    comm.Allreduce(
                        [synchronizer.local_data[redname][cname], mpi.DOUBLE],
                        [synchronizer.global_data[redname][cname], mpi.DOUBLE],
                        op=mpi.SUM,
                    )
                    logging.debug(" post-reduce %s on rank %d" % (redname, rank))
                    if (
                        synchronizer.enable_side_gig
            ...
            synchronizer.comms["ROOT"].Allreduce(
                [synchronizer.sleep_secs, mpi.DOUBLE],
