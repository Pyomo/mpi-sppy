...
def worker_bee(synchronizer, buzz=1e6, sting=100.0):
    ...
    global_concat = {
        "FirstReduce": {
            "ROOT": np.zeros(len(local_concat["FirstReduce"]["ROOT"]), dtype="d")
        }
    }
    ...
    synchronizer.compute_global_data(
        local_concat, global_concat, rednames=["FirstReduce"], enable_side_gig=True
    )

    while global_iters_sofar < buzz:
        # attempt at less sleep, maybe (but don't do it twice in a row)
        if np.random.uniform() > 0.1 and old_sleep[0] == synchronizer.sleep_secs[0]:
            old_sleep[0] = synchronizer.sleep_secs[0]
            synchronizer.sleep_secs[0] /= 10
            logging.debug(
                "TRYING to reduce sleep to {} from rank={}".format(
                    synchronizer.sleep_secs[0], rank
                )
            )
        elif old_sleep[0] != synchronizer.sleep_secs[0]:
            synchronizer.sleep_secs[0] = old_sleep[0]
            logging.debug(
                "putting sleep back to {} from rank={}".format(
                    synchronizer.sleep_secs[0], rank
                )
            )
        ...
        if rank == 0:
            logging.debug(
                "rank 0: iterstodo {} iters sofar {} sum_so_far {}=".format(
                    localiterstodo, local_iters_sofar, local_sum_sofar
                )
            )
        ...
        if rank == 0:
            logging.debug(
                "   rank 0: global_iters {} global_sum_so_far {}".format(
                    global_iters_sofar, global_sum
                )
            )
    ...
    if rank == 0:
        ...
        print(
            "Based on {} iterations, the average was {}".format(
                global_iters_sofar, global_sum / global_iters_sofar
            )
        )
...
def side_gig(synchro, msg=None):
    ...
    for cname in synchro.Lens[prevredname]:
        if not np.array_equal(
            side_gig.prev_red_prev_concat[cname],
    ...
    logging.debug(
        "In case you are curious, before this listener call to"
        + "side_gig, the value of the secondreduce was {} on rank {}".format(
            lastsideresult, rank
        )
    )
...
####### Main #######
usemsg = (
    "usage: python demo_listener_util.py iters sleep seed; e.g.,\n"
...
synchronizer = listener_util.Synchronizer(
    comms={"ROOT": fullcomm},
    Lens=Lens,
    work_fct=worker_bee,
    rank=rank,
...
Lens = collections.OrderedDict(
    {"FirstReduce": {"ROOT": 2 + n_proc}, "SecondReduce": {"ROOT": 1}}
)
listener_gigs = {"FirstReduce": (side_gig, kwargs), "SecondReduce": None}
