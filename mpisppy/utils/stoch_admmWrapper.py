...
def _consensus_vars_number_creator(consensus_vars):
    ...
    for subproblem in consensus_vars:
        for var_stage_tuple in consensus_vars[subproblem]:
            ...
            if (
                var not in consensus_vars_number
            ):  # instanciates consensus_vars_number[var]
...
class Stoch_AdmmWrapper:  # add scenario_tree
    ...
    def __init__(
        ...
        scen_tree = sputils._ScenTree(
            ["ROOT"], all_admm_stoch_subproblem_scenario_names
        )
        assert (
            mpicomm.Get_size() % n_cylinders == 0
        ...
        self.local_admm_stoch_subproblem_scenarios_names = [
            all_admm_stoch_subproblem_scenario_names[i]
            for i in _rank_slices[cylinder_rank]
        ]
        ...
        self.split_admm_stoch_subproblem_scenario_name = (
            split_admm_stoch_subproblem_scenario_name
        )
        ...
        self.all_nodenames = self.create_node_names(
            num_admm_subproblems=len(admm_subproblem_names),
            num_stoch_scens=len(stoch_scenario_names),
        )
    ...
    def create_node_names(self, num_admm_subproblems, num_stoch_scens):
        if self.BFs is not None:  # already multi-stage problem initially
            ...
            )  # Adds the last stage with admm_subproblems
    ...
    def assign_variable_probs(self, verbose=False):
        ...
        all_consensus_vars = {
            var_stage_tuple[0]: var_stage_tuple[1]
            for admm_subproblem_names in self.consensus_vars
            for var_stage_tuple in self.consensus_vars[admm_subproblem_names]
        }
        error_list1 = []
        error_list2 = []
        for sname, s in self.local_admm_stoch_subproblem_scenarios.items():
            if verbose:
                print(
                    f"stoch_admmWrapper.assign_variable_probs is processing scenario: {sname}"
                )
            admm_subproblem_name = self.split_admm_stoch_subproblem_scenario_name(
                sname
            )[0]
            ...
            ), f"the scenario {sname} doesn't have any _mpisppy_probability attribute"
            if (
                s._mpisppy_probability == "uniform"
            ):  # if there is a two-stage scenario defined by sputils.attach_root_node
                ...
                if var_stage_tuple in self.consensus_vars[admm_subproblem_name]:
                    if v is not None:
                        ...
                        if stage == depth:
                            ...
                            prob_node = np.prod(
                                [
                                    s._mpisppy_node_list[ancestor_stage - 1].cond_prob
                                    for ancestor_stage in range(1, stage + 1)
                                ]
                            )
                        ...
                        self.varprob_dict[s].append(
                            (id(v), cond_prob / (self.consensus_vars_number[vstr]))
                        )
                    ...
                    if v is None:
                        ...
                        v2str = vstr.replace("[", "__").replace(
                            "]", "__"
                        )  # To distinguish the consensus_vars fixed at 0
                        v = pyo.Var()
            ...
            ), f"the scenario {sname} doesn't have any _mpisppy_node_list attribute"
            parent = s._mpisppy_node_list[-1]
            admm_subproblem_name, stoch_scenario_name = (
                self.split_admm_stoch_subproblem_scenario_name(sname)
            )
            num_scen = self.stoch_scenario_names.index(stoch_scenario_name)
            if self.BFs is not None:
                node_num = (
                    num_scen % self.BFs[-2]
                )  # BFs has already been updated to include the last rank with the leaves
            else:
                node_num = num_scen
            node_name = parent.name + "_" + str(node_num)

            # could be more efficient with a dictionary rather than index
            s._mpisppy_node_list.append(
                scenario_tree.ScenarioNode(
                    node_name,
                    1
                    / self.number_admm_subproblems,  # branching probability at this node
                    parent.stage + 1,  # The stage is the stage of the previous leaves
                    pyo.Expression(
                        expr=0
                    ),  # The cost is spread on the branches which are the subproblems
                    varlist[depth - 1],
                    s,
                )
            )
            ...
            self.local_admm_stoch_subproblem_scenarios[sname] = (
                s  # I don't know whether this changes anything
            )
    ...
    def admmWrapper_scenario_creator(self, admm_stoch_subproblem_scenario_name):
        scenario = self.local_admm_stoch_subproblem_scenarios[
