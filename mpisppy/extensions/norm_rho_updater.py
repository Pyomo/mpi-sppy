...
class NormRhoUpdater(mpisppy.extensions.extension.Extension):
    def __init__(self, ph):
        ...
        self.norm_rho_options = (
            ph.options["norm_rho_options"]
            if "norm_rho_options" in ph.options
            else dict()
        )
    ...
    def _set_options(self):
        ...
        for attr_name, opt_name in _attr_to_option_name_map.items():
            setattr(
                ...
                else _norm_rho_defaults[opt_name],
            )
    ...
    def _compute_dual_residual_norm(self, ph):
        ...
        for s in ph.local_scenarios.values():
            for ndn_i in s._mpisppy_data.nonant_indices:
                dual_resid[ndn_i] = s._mpisppy_model.rho[ndn_i].value * math.fabs(
                    s._mpisppy_model.xbars[ndn_i].value - self._prev_avg[ndn_i]
                )
        return dual_resid
        ...
        if not self._prev_avg:
            ...
            for s in ph.local_scenarios.values():
                for ndn_i, rho in s._mpisppy_model.rho.items():
                    ...
                    action = None
                    if (primal_resid > primal_dual_difference_factor * dual_resid) and (
                        primal_resid > self._tol
                    ):
                        rho._value *= self._rho_increase
                        action = "Increasing"
                    elif (
                        dual_resid > primal_dual_difference_factor * primal_resid
