...
class XhatSpecific(mpisppy.extensions.xhatbase.XhatBase):
    ...
    # ==========
    def xhat_tryit(self, xhat_scenario_dict, verbose=False, restore_nonants=True):
        """If your rank has
    ...
    def post_everything(self):
        ...
        obj = self.xhat_tryit(
            xhat_scenario_dict, verbose=self.verbose, restore_nonants=restore_nonants
        )
        self.opt.reenable_W_and_prox()
        # to make available to tester
        self._xhat_specific_obj_final = obj
        self.xhat_common_post_everything(
            "xhat specified scenario", obj, xhat_scenario_dict, restore_nonants
        )
