...
class MultRhoUpdater(mpisppy.extensions.extension.Extension):
    def __init__(self, ph):
        ...
        self.mult_rho_options = (
            ph.options["mult_rho_options"]
            if "mult_rho_options" in ph.options
            else dict()
        )
    ...
    def _set_options(self):
        ...
        for attr_name, opt_name in _attr_to_option_name_map.items():
            setattr(
                ...
                else _mult_rho_defaults[opt_name],
            )
    ...
    def _attach_rho_ratio_data(self, ph, conv):
        ...
        if not self.ph.multistage:
            ...
            self._first_rho = {
                ndn_i: rho._value for ndn_i, rho in s._mpisppy_model.rho.items()
            }
        else:
    ...
    def miditer(self):
        ph = self.ph
        ph_iter = ph._PHIter
        if (self._stop_iter is not None and ph_iter > self._stop_iter) or (
            self._start_iter is not None and ph_iter < self._start_iter
        ):
            return
        conv = self._conv()
        ...
        if self._first_rho is None:
            ...
            if ph.cylinder_rank == 0:
                print(
                    f"MultRhoUpdater iter={ph_iter}; {ndn_i} now has value {rho._value}"
                )
