...
class PHTracker(Extension):
    ...
    def __init__(self, opt):
        ...
        self._track_var_to_func = {
            "gaps": {"track": self.add_gaps, "finalize": self.plot_xbars_bounds_gaps},
            "bounds": {
                "track": self.add_bounds,
                "finalize": self.plot_xbars_bounds_gaps,
            },
            "nonants": {
                "track": self.add_nonants,
                "finalize": self.plot_nonants_sgaps_duals,
            },
            "duals": {
                "track": self.add_duals,
                "finalize": self.plot_nonants_sgaps_duals,
            },
            "xbars": {"track": self.add_xbars, "finalize": self.plot_xbars_bounds_gaps},
            "scen_gaps": {
                "track": self.add_scen_gaps,
                "finalize": self.plot_nonants_sgaps_duals,
            },
            "reduced_costs": {"track": self.add_rc, "finalize": self.plot_rc},
        }
    ...
    def verify_tracking(self):
        ...
        if "gaps" in self.track_dict or "bounds" in self.track_dict:
            if (
                isinstance(self.spcomm, Spoke)
                and not hasattr(self.spcomm, "hub_outer_bound")
                and not hasattr(self.spcomm, "hub_inner_bound")
            ):
                raise RuntimeError(
                    "Cannot access hub gaps without passing" " them to spcomm"
                )
    ...
    def get_scen_colnames(self):
        ...
        scen_names = [
            (sname, b)
            for sname in self.opt.local_scenarios.keys()
            for b in ["ub", "lb"]
        ]
        return scen_names
    ...
    def add_gaps(self, final=False):
        ...
        if isinstance(self.spcomm, Spoke):
            ...
            if ConvergerSpokeType.OUTER_BOUND in self.spcomm.converger_spoke_types:
                spoke_abs_gap = (
                    spoke_bound - hub_outer_bound
                    if self.opt.is_minimizing
                    else hub_outer_bound - spoke_bound
                )
                ...
                spoke_abs_gap = (
                    hub_inner_bound - spoke_bound
                    if self.opt.is_minimizing
                    else spoke_bound - hub_inner_bound
                )
    ...
    def add_duals(self, final=False):
        ...
        local_duals_data = {
            (sname, var.name): scenario._mpisppy_model.W[node.name, ix]._value
            for (sname, scenario) in self.opt.local_scenarios.items()
    ...
    def add_xbars(self, final=False):
        ...
        xbars = {
            var.name: scenario._mpisppy_model.xbars[node.name, ix]._value
            for node in scenario._mpisppy_node_list
            for (ix, var) in enumerate(node.nonant_vardata_list)
        }
    ...
    def plot_nonants_sgaps_duals(self, var):
        ...
        df = df.replace([np.inf, -np.inf], np.nan)
        df.columns = [
            col if col == "iteration" else ast.literal_eval(col) for col in df.columns
        ]
