...
class CrossScenarioExtension(Extension):
    def __init__(self, spbase_object):
        ...
        if self.opt.multistage:
            raise RuntimeError(
                "CrossScenarioExtension only supports " "two-stage models at this time"
            )

        opt = self.opt
        if (
            "cross_scen_options" in opt.options
            and "check_bound_improve_iterations" in opt.options["cross_scen_options"]
        ):
            self.check_bound_iterations = opt.options["cross_scen_options"][
    ...
    def _check_bound(self):
        ...
        teeme = "tee-rank0-solves" in opt.options and opt.options["tee-rank0-solves"]
        opt.solve_loop(
        ...
        local_obs = np.fromiter(
            (s._mpisppy_data.outer_bound for s in opt.local_subproblems.values()),
            dtype="d",
    ...
    def get_from_cross_cuts(self):
        ...
        receive_buffer = np.empty(
            spcomm.remote_lengths[idx - 1] + 1, dtype="d"
        )  # Must be doubles
        ...
        if opt.bundling:
            for bn, b in opt.local_subproblems.items():
                ...
                for idx, k in enumerate(opt.all_scenario_names):
                    ...
                    cut_expr = LinearExpression(
                        constant=linear_const,
            ...
            for sn, s in opt.local_subproblems.items():
                ...
                for idx, k in enumerate(opt.all_scenario_names):
                    ...
                    s._mpisppy_model.benders_cuts[outer_iter, k] = (None, cut_expr, 0.0)
                    if persistent_solver:
    ...
    def pre_iter0(self):
        if self.opt.multistage:
            raise RuntimeError(
                "CrossScenarioExtension does not support "
    ...
    def post_iter0(self):
        ...
        if (
            "cross_scen_options" in opt.options
            and "valid_eta_bound" in opt.options["cross_scen_options"]
        ):
            ...
            lb = (-sys.maxsize - 1) * 1.0 / len(opt.all_scenario_names)
        ...
        for k, s in opt.local_subproblems.items():
            s._mpisppy_model.eta = pyo.Var(
                opt.all_scenario_names, initialize=_eta_init, bounds=_eta_bounds
            )
            ...
            if bundling:  ## create a refence to eta on each subproblem
                for sn in s.scen_list:
                    ...
                    scenario._mpisppy_model.eta = {
                        k: s._mpisppy_model.eta[k] for k in opt.all_scenario_names
                    }
        ...
        for k, s in opt.local_subproblems.items():
            ...
            if bundling:
                ...
                for sn in s.scen_list:
                    nonant_vardata_list.extend(
                        opt.local_scenarios[sn]
                        ._mpisppy_node_list[0]
                        .nonant_vardata_list
                    )
            else:
            ...
            if repn.quadratic_vars:
                expr += pyo.quicksum(
                    (
                        coef * x * y
                        for coef, (x, y) in zip(quadratic_coefs, repn.quadratic_vars)
                    )
                )
            ...
            if opt.is_minimizing:
                s._mpisppy_model.EF_Obj = pyo.Objective(
                    expr=s._mpisppy_model.EF_obj, sense=pyo.minimize
                )
            else:
                s._mpisppy_model.EF_Obj = pyo.Objective(
                    expr=-s._mpisppy_model.EF_obj, sense=pyo.maximize
                )
            s._mpisppy_model.EF_Obj.deactivate()
    ...
    def miditer(self):
        ...
        check = (
            (self.check_bound_iterations is not None)
            and self.any_cuts
            and (
                (self.iter_at_cur_ib == self.check_bound_iterations)
                or (self.iter_at_cur_ib > self.check_bound_iterations and ob_new)
                or (
                    (self.iter_since_last_check % self.check_bound_iterations == 0)
                    and self.new_cuts
                )
            )
        )
