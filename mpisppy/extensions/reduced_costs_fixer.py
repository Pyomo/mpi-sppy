...
class ReducedCostsFixer(Extension):
    def __init__(self, spobj):
        ...
        if not (self._use_rc_bt or self._use_rc_fixer) and self.opt.cylinder_rank == 0:
            print(
                "Warning: ReducedCostsFixer will be idle. Enable use_rc_bt or use_rc_fixer in options."
            )
    ...
    def sync_with_spokes(self):
        ...
        if serial_number > self._last_serial_number:
            ...
            reduced_costs = spcomm.outerbound_receive_buffers[idx][
                1 : 1 + self.nonant_length
            ]
    ...
    def reduced_costs_bounds_tightening(self, reduced_costs, this_outer_bound):
        ...
        if np.isinf(inner_bound) or np.isinf(outer_bound):
            if self.opt.cylinder_rank == 0 and self.verbose:
                print(
                    "Bounds tightened by reduced cost: 0 (inner or outer bound not available)"
                )
            return
                ...
                for ci, (ndn_i, xvar) in enumerate(
                    ...
                    if is_minimizing:
                        ...
                        if this_expected_rc > 0 + self.zero_rc_tol:
                            new_ub = (
                                xvar.lb + (inner_bound - outer_bound) / this_expected_rc
                            )
                            old_ub = xvar.ub
                            if new_ub < old_ub:
                                ...
                                if self.debug and self.opt.cylinder_rank == 0:
                                    print(
                                        f"tightening ub of var {xvar.name} to {new_ub} from {old_ub}; reduced cost is {this_expected_rc}"
                                    )
                                update_var = True
                        ...
                        # var at ub
                            ...
                            new_lb = (
                                xvar.ub + (inner_bound - outer_bound) / this_expected_rc
                            )
                            old_lb = xvar.lb
                            if new_lb > old_lb:
                                if ndn_i in self._integer_nonants:
                                ...
                                if self.debug and self.opt.cylinder_rank == 0:
                                    print(
                                        f"tightening lb of var {xvar.name} to {new_lb} from {old_lb}; reduced cost is {this_expected_rc}"
                                    )
                                update_var = True
                                bounds_reduced_this_iter += 1
                                tightened_this_scenario += 1
                    # maximization
                        ...
                        if this_expected_rc < 0 - self.zero_rc_tol:
                            new_ub = (
                                xvar.lb - (outer_bound - inner_bound) / this_expected_rc
                            )
                            old_ub = xvar.ub
                            if new_ub < old_ub:
                                if ndn_i in self._integer_nonants:
                        ...
                        # var at ub
                            ...
                            new_lb = (
                                xvar.ub - (outer_bound - inner_bound) / this_expected_rc
                            )
                            old_lb = xvar.lb
                            if new_lb > old_lb:
                                if ndn_i in self._integer_nonants:
        ...
        total_bounds_tightened = bounds_reduced_this_iter / len(
            self.opt.local_scenarios
        )
        if self.opt.cylinder_rank == 0 and self.verbose:
            print(
                f"Bounds tightened by reduced cost: {int(round(total_bounds_tightened))}/{self.nonant_length}"
            )
    ...
    def reduced_costs_fixing(self, reduced_costs):
                ...
                for ci, (ndn_i, xvar) in enumerate(
                    ...
                    if np.isnan(this_expected_rc):
                        ...
                        if xvar.fixed:
                            ...
                            if self.debug and self.opt.cylinder_rank == 0:
                                print(
                                    f"unfixing var {xvar.name}; not converged in LP-LR"
                                )
                        ...
                        if xvar.fixed:
                            if this_expected_rc <= target:
                                ...
                                if self.debug and self.opt.cylinder_rank == 0:
                                    print(
                                        f"unfixing var {xvar.name}; reduced cost is zero/below target in LP-LR"
                                    )
                        else:
                            ...
                            if this_expected_rc >= target:
                                if self.opt.is_minimizing:
                                    ...
                                    if (reduced_costs[ci] > 0 + self.zero_rc_tol) and (
                                        xb - xvar.lb <= self.bound_tol
                                    ):
                                        xvar.fix(xvar.lb)
                                        if self.debug and self.opt.cylinder_rank == 0:
                                            print(
                                                f"fixing var {xvar.name} to lb {xvar.lb}; reduced cost is {reduced_costs[ci]} LP-LR"
                                            )
                                        update_var = True
                                        raw_fixed_this_iter += 1
                                    elif (
                                        reduced_costs[ci] < 0 - self.zero_rc_tol
                                    ) and (xvar.ub - xb <= self.bound_tol):
                                        xvar.fix(xvar.ub)
                                        if self.debug and self.opt.cylinder_rank == 0:
                                            print(
                                                f"fixing var {xvar.name} to ub {xvar.ub}; reduced cost is {reduced_costs[ci]} LP-LR"
                                            )
                                        update_var = True
                                        raw_fixed_this_iter += 1
                                    else:
        ...
        self._heuristic_fixed_vars += raw_fixed_this_iter / len(
            self.opt.local_scenarios
        )
        if self.opt.cylinder_rank == 0 and self.verbose:
            print(
                f"Total unique vars fixed by heuristic: {int(round(self._heuristic_fixed_vars))}/{self.nonant_length}"
            )
