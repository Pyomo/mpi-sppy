...
class XhatClosest(mpisppy.extensions.xhatbase.XhatBase):
    ...
    def xhat_closest_to_xbar(self, verbose=False, restore_nonants=True):
        ...
        for k, s in self.opt.local_scenarios.items():
            ...
            for ndn_i, xvar in s._mpisppy_data.nonant_indices.items():
                ...
                variance = pyo.value(s._mpisppy_model.xsqbars[ndn_i]) - pyo.value(
                    s._mpisppy_model.xbars[ndn_i]
                ) * pyo.value(s._mpisppy_model.xbars[ndn_i])
                if variance > 0:
        ...
        self.comms["ROOT"].Allreduce(
            [localmindist, mpi.DOUBLE], [globalmindist, mpi.DOUBLE], op=mpi.MIN
        )
        ...
        self.comms["ROOT"].Allreduce(
            [localwinrank, mpi.DOUBLE], [globalwinrank, mpi.DOUBLE], op=mpi.MAX
        )
    ...
    def post_everything(self):
        ...
        obj, srcsname = self.xhat_closest_to_xbar(
            verbose=self.verbose, restore_nonants=restore_nonants
        )
        self.opt.reenable_W_and_prox()
        self.xhat_common_post_everything(
            "closest to xbar", obj, srcsname, restore_nonants
        )
